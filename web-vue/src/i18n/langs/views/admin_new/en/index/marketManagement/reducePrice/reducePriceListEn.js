export const en = {
  // tabs
  tabInfo: [
    {
      title: 'All',
      name: '0'
    },
    {
      title: 'In Progress',
      name: '1'
    },
    {
      title: 'Not Started',
      name: '2'
    },
    {
      title: 'Overdue',
      name: '3'
    },
    {
      title: 'Disable',
      name: '4'
    }
  ],

  // 通用
  reducePrice: 'reduce price',
  expressDelivery: 'including courier fee',
  goodsInfo: 'goods information',

  // 限时降价活动列表,
  addReducePrice: 'create a reduce price activity',
  goodsAmount: 'goods amount',
  paidOrderNumber: 'paid order number',
  paidUserNumber: 'paid user number',
  totalMoneyPaid: 'total money paid',
  orderDetails: 'order details',

  // 活动添加页
  actNameTip: 'Only used as a business record, users will not see this name',
  repeatByCycle: 'Repeat by cycle',
  edit: 'Edit',
  noLimit: 'not limited',
  preTime: 'Event notice',
  preTimeTip1: 'Pre event information will be displayed in the product details before the event',
  preTimeTip2: 'View examples',
  preTimeTip3: 'Before the event',
  preTimeTip4: 'Hours for advance notice',
  preTimeTip5: 'Pre notice after activity creation',
  preTimeTip6: 'No activity notice',
  activityTag: 'Simultaneous tagging',
  activityTagTip1: 'Tag active users',
  activityTagTip2: 'Users who successfully place an order and complete the payment are regarded as participating activities',
  activityTagTip3: 'Select label',
  activityTagTip4: 'Up to 3 labels can be set',
  purchaseQuantity: 'purchase quantity',
  limitQuantity: 'limit quantity',
  limitQuantityTip: 'After the limit number exceeds the limit purchase quantity, the buyer cannot continue to add the purchase price.',
  piece: 'piece',
  setDiscount: 'set discount',
  batch: 'batch',
  discount: 'discount',
  priceReduction: 'price reduction',
  priceAfterDiscount: 'price after discount',
  originalPrice: 'original price',
  stock: 'stock',
  productReducePriceTip: 'specifications price reduction',

  // 校验
  validActName: 'Please fill in the event name！',
  validActDate: 'Please fill in the validity period of the event！',
  validActGoods: 'Please set up event products！',
  validActGoodsPrice: 'Please set the full product activity price！',

  batchValidPrice1: 'The price reduction must be less than the original price！',
  batchValidPrice2: 'After the price reduction, the amount must not be less than 0！',
  batchValidPrice3: 'This item cannot be reduced in price！',
  batchValidSet: 'Please tick the item and try again！',
  batchValidDiscount1: 'Please enter the number of discounts！',
  batchValidDiscount2: 'Discount can only be entered between 0-10！',
  batchValidSetType: 'Please select a batch type！',

  // 活动数据效果
  legendData: [
    'Total amount paid for activities',
    'Total amount of event discount',
    'Cost-effectiveness ratio',
    'Number of payment orders',
    'Number of items for payment',
    'Number of new customers',
    'Number of households of Laocheng'
  ],
  paidOrderNumberTip: 'Number of payment orders (including refund) brought by the event',
  paidGoodsNumber: 'Number of items for payment',
  paidGoodsNumberTip: 'Number of items for active payment'
}
