...
<script>
import { searchAccountByOneRequest, editShopRequest } from '@/api/system/shopList.js'
export default {
  name: 'editShop',
  data () {
    return {
      ...
      shopId: this.editParam.shopId,
      phonereg: /^1[3|7|8]\d{9}$|^19[8-9]\d{8}$|^166\d{8}|^15[0-3|5-9]\d{8}|^14[5|7]\d{8}$/
    }
  },
  props: ['editParam'],
  mounted () {
    console.log('接收的', this.shopId)
    this.getList()
  },
  methods: {
    getList () {
      searchAccountByOneRequest(this.shopId).then(res => {
        console.log(res)
        ...
        if (res.error === 0) {
          ...
          console.log(this.Data.userName)
          // 数据库先默认第一个
          this.Data.dbConfigId = this.dbVersion[1].value
        } else {
    ...
    // 添加商家账户
    save () {
      this.exchangeCheckBox()
      editShopRequest(this.Data).then(res => {
        console.log(res)
        this.centerDialogVisible = false
        if (res.error === 0) {
    ...
    checkBoxExchange () {
      if (this.Data.isEnabled === 1) {
        this.Data.isEnabled = true
      } if (this.Data.hidBottom === 1) {
        this.Data.hidBottom = true
      } if (this.Data.isEnabled === 0) {
        this.Data.isEnabled = false
      } if (this.Data.hidBottom === 0) {
        this.Data.hidBottom = false
      }
    },
    exchangeCheckBox () {
      if (this.Data.isEnabled === true) {
        this.Data.isEnabled = 1
      } if (this.Data.hidBottom === true) {
        this.Data.hidBottom = 1
      }
      if (this.Data.isEnabled === false) {
        this.Data.isEnabled = 0
      } if (this.Data.hidBottom === false) {
        this.Data.hidBottom = 0
      }
    },
    backHome () {
      console.log('点了')
