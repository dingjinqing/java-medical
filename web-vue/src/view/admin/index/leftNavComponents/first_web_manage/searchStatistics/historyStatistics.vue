...
import {getHistory} from '@/api/admin/firstWebManage/searchStatistics/searchStatistics.js'
...
export default {
  ...
  methods: {
    ...
    // 页面初始化数据
    loadData () {
      getHistory(this.param).then(res => {
        console.log('搜搜历史统计：', res)
        if (res.error === 0) {
          this.isOpenHistory = res.content.isOpenHistory
