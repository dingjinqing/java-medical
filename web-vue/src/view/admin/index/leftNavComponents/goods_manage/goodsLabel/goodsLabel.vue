<template>
  <div
    class="china_checkbox"
    v-if="JSON.stringify(REGION_DATA) != '{}'"
  >
    <el-checkbox-group
      @change="pidChange"
      v-model="provinceId"
    >
      ...
      <div
        class="pid_ck_for"
        v-for="(pname, pid) in REGION_DATA[86]"
        :key="pid"
      >
        <el-checkbox
          class="ck_box"
        ...
        ><span
          ...
          <el-checkbox-group
            class="c_ck_box"
            v-model="cityId"
            @change="checked=>cidChange(checked,pid)"
          >
            <!-- 此处渲染市级 -->
            <div
              v-if="CityList[pid]"
            ...
            >
              <div
                class="cid_ck_for"
                v-for="(cname, cid) in CityList[pid]"
                :key="cid"
              >
                <el-checkbox
                  class="cid_ck_box"
                ...
                ><span
                  ...
                  <el-checkbox-group
                    class="a_ck_box"
                    v-model="districtId"
                    @change="checked=>aidChange(checked,pid,cid)"
                  >
                    <!-- 此处渲染区县 -->
                    <div
                      v-if="AreaList[cid]"
                    ...
                    >
                      <el-checkbox
                        v-for="(aname, aid) in AreaList[cid]"
                        :key="aid"
                        :label="aid"
                      ><span
                          class="aid_title"
                          :class="'aid'+aid"
                        >{{aname}}</span></el-checkbox>
                      <!-- 此处关闭区县下拉菜单 -->
                      <div class="clear_box">
                        <el-checkbox @click.native.prevent="clearBox($event, '.cid'+cid)">关闭</el-checkbox>
                      </div>
                    </div>
                  </el-checkbox-group>
                </el-checkbox>
                ...
                <el-checkbox
                  :class="'triangle_box'"
                  @click.native.prevent="changeStyle($event, ('.cid'+cid), '.a_ck_box', 'AreaList', cid)"
                >
                  <div
                    class="triangle-down"
                    v-if="REGION_DATA[cid]"
                  ></div>
                </el-checkbox>
              </div>
              <!-- 此处关闭市级下拉菜单 -->
              <div class="clear_box">
                <el-checkbox @click.native.prevent="clearBox($event, '.pid'+pid)">关闭</el-checkbox>
              </div>
            </div>
          </el-checkbox-group>
        </el-checkbox>
        ...
        <el-checkbox
          :class="'triangle_box'"
          @click.native.prevent="changeStyle($event,('.pid'+pid), '.c_ck_box', 'CityList', pid)"
        >
...
<script>
import chinaData from '@/util/data'
export default {
  ...
  methods: {
    ...
    // 匹配省级变动
    pidChange (v) {
      const change = this.isRemove(this.old_provinceId, v, 'cityId', 'districtId', 'provinceId')

      if (change.length) this.old_provinceId = [...v]
      change.forEach(e => {
        const keys = Object.keys(this.REGION_DATA[e])
        this.setCk('cityId', keys, 'districtId')
      })
    },

    // 匹配市级变动
    cidChange (v, pid) {
      const change = this.isRemove(this.old_cityId, v, 'districtId', null, 'cityId')
      if (!change.length) return
      this.old_cityId = [...v]
      if (!~this.provinceId.indexOf(pid)) this.provinceId.push(pid)
      change.forEach(e => {
        const keys = Object.keys(this.REGION_DATA[e])
        this.setCk('districtId', keys, null)
      })
    },
    ...
    aidChange (v, pid, cid) {
      ...
      if (v.length > this.old_districtId.length) {
        if (!~this.provinceId.indexOf(pid)) this.provinceId.push(pid)
    ...
    remove (name, keys, nextName) {
      ...
      if (name === 'districtId') return false
      keys.forEach(e => {
