...
<script>
import { goodsListApi, downloadFailExportApi, downloadModuleApi, uploadGoodsApi, isImportFinish } from '@/api/admin/goodsManage/goodsImport/goodsImport.js'
...
export default {
  ...
  methods: {
    ...
    queryIsFinish (batchId) {
      let that = this
      setTimeout(function () {
        isImportFinish(batchId).then(res => {
          if (res.content === true) {
            that.vLoading = false
