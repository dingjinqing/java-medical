...
import { seckillOrderListExport, getSeckillExportTotalRows } from '@/api/admin/marketManage/seckill.js'
export default {
  data () {
    return {
      totalRows: 0,
  ...
  methods: {
    initData () {
      ...
      getSeckillExportTotalRows(this.param).then(res => {
        if (res.error === 0) {
    ...
    confirm () {
      ...
      seckillOrderListExport(this.param).then(res => {
        console.log(res, 'excle-data')
        let fileName = localStorage.getItem('V-content-disposition')
        fileName = fileName && fileName !== 'undefined' ? fileName.split(';')[1].split('=')[1] : '秒杀订单导出.xlsx'
        this.loading = false
