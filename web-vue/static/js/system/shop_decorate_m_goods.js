...
var m_goods = {
	init_ev_el: function(el,data) {
		...
		//var double_flag = isNaN(parseInt(data.col_type)) || parseInt(data.col_type) == 1;//默认双列展示
		// let checkID = [];
		// data.goods_items.forEach((element,index) => {
		// 	checkID.push(element.goods_id);
		// });
		// let goods_check_id = checkID.join(',');
		// console.log(goods_check_id)
		// util.ajax_json('/admin/purchase/goods/list', function (response) {
		// 	console.log()
		// },{select_id: goods_check_id});
        ...
        function callbackInitGoods() {
            ...
            if (good_flag == true && parseInt(data.col_type) == 0) {
                for (var i in initGoods) {
                    m_goods.add_single_good_el(el, initGoods[i], data.goods_display,data.if_radius,data.goods_module_style);
                }
            } else if (good_flag == true && parseInt(data.col_type) == 2) {
                for (var i = 0; i < initGoods.length; i += 3) {
                    m_goods.add_three_good_el(el, initGoods[i], initGoods[i+1], initGoods[i + 2],data.if_radius,data.goods_module_style);
                }
            } else if(good_flag == true && parseInt(data.col_type) == 3){
                // for (var i = 0; i < initGoods.length; i += 3) {
                    m_goods.add_some_good_el(el, initGoods[0], initGoods[1], initGoods[2],data.if_radius,data.goods_module_style);
                // }
			}else {
                for (var i = 0; i < initGoods.length; i += 2) {
                    m_goods.add_double_good_el(el, initGoods[i], initGoods[i + 1],data.if_radius,data.goods_module_style);
                }
            }
        }
	...
	add_single_good_el: function(el,d,goods_display,if_radius,goods_module_style){
		...
		if(d.market_price) e.find(".p_goods_item .item-price .mk_price:eq(0)").text("￥"+d.market_price);
	...
	add_double_good_el: function(el,d1,d2,if_radius,goods_module_style){
		...
		if(d1.market_price) e.find(".p_goods_item1 .item-price .mk_price:eq(0)").text("￥"+d1.market_price);
		...
		if(d2){
			...
			if(d2.market_price != 0) e.find(".p_goods_item2 .item-price .mk_price:eq(0)").text("￥"+d2.market_price);
	...
	add_three_good_el: function(el,d1,d2,d3,if_radius,goods_module_style){
		...
		if(d1.market_price) e.find(".t_goods_item1 .item-price .mk_price:eq(0)").text("￥"+d1.market_price);
		...
		if(d2){
			...
			if(d2.market_price != 0) e.find(".t_goods_item2 .item-price .mk_price:eq(0)").text("￥"+d2.market_price);
		...
		if(d3){
			...
			if(d3.market_price != 0) e.find(".t_goods_item3 .item-price .mk_price:eq(0)").text("￥"+d3.market_price);
    ...
    add_some_good_el:function (el,d1,d2,d3,if_radius,goods_module_style) {
	...
	fill_edit_el: function(el,d){
		...
		//el.find("[name='col_type']").val(parseInt(d.col_type) < 2 ? d.col_type : 2);
	...
	get_data: function(){
		...
		d.hide_label=el.find("#hide_goods_label input[type=checkbox]:checked").val();
