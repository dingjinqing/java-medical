(function ($) {
    ...
    function set_cat_id() {
        ...
        layer.open({
            type: 1
            , title: ["移动到新的文件夹", 'text-align:center;padding: 0px;']
            , offset: 'auto'
            , content: "<div style='padding:15px'>"+cat_el[0].outerHTML+"</div>"
            , btn: ['确定', '取消']
            , btnAlign: 'r' //按钮居右
    ...
    function operate_submit(op) {
        ...
        if (show_type == 'list' && el_chk.length == 0
            || show_type == 'bigimg' && el_bigvideo_chk.length == 0) {
    ...
    $("#bat_remove_video").click(function () {
        remove_item();
    });
    ...
    $("#bigvideo_data li img").click(function () {
        ...
        var o = {
            ...
            video_path: $(el).attr('video_path'),snapshot_url: $(el).attr('snapshot_url'),
