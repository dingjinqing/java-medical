...
var m_goods_group = {
     ...
     * 商品列表样式
     */
    add_group_module_el:function(el,d,if_radius,module_style,position_style,cart_btn,cart_btn_choose,show_price){
       ...
       if(position_style == 0){
        ...
        for(var i = 0;i < goods_len;i++) {
            ...
            if(d.market_price[i] && d.market_price[i] != 0 && d.cart_btn != 0){
                emore.find(".group-origin-price").removeClass("hide");
                emore.find(".group-origin-price").text('￥' + d.market_price[i]);
            }else{
                emore.find(".group-origin-price").addClass("hide");
            }
            if(d.cart_btn == 0){
                switch(parseInt(d.cart_btn_choose))
                {
                    case 0:
                        emore.find(' .cart_btn:eq(0)').css('display','inline-block');
                        break;
                    case 1:
                        emore.find(' .cart_btn:eq(1)').css('display','inline-block');
                        break;
                    case 2:
                        emore.find(' .cart_btn:eq(2)').css('display','inline-block');
                        break;
                    case 3:
                        emore.find(' .cart_btn:eq(3)').css('display','inline-block');
                        break;
                }
            }else{
        ...
        if(d.show_price == 1){
            e.find(".group-goods-price").addClass("hide");
            e.find(".group-goods-price").addClass("hide");
            e.find(".group-origin-price").addClass("hide");
            e.find(".group-origin-price").addClass("hide");
        }
        if(d.show_name == 1){
            e.find(".goods-title").addClass("hide");
            e.find(".goods-title").addClass("hide");
        }
    },
    ...
    fill_edit_el: function(el,d){
       ...
       if(d.menu_style == 1){
        ...
        if(d.show_name != 1)  el.find("#shop_goods_name").attr("checked",true);
        if(d.show_price != 1)  el.find("#shop_goods_price").attr("checked",true);
        if(d.cart_btn == 0){
            el.find("#shop_cart_btn").attr("checked",true);
            el.find('.btn_cart').show();
            el.find('input[name="cart_checked"]').each(function(item,index){
        ...
        el.find("input[name='position_style']").click(function () {
            var cur_m = manager.get_cur_edit_module();
            var cur_data = cur_m.get_data();
            if($(this).val() == 0){
                el.find(".left_none").show();
                if(cur_data.shop_style == 3 || cur_data.shop_style == 5){
                    el.find('#shop_cart_3').attr('disabled',true);
                    el.find('#shop_cart_4').attr('disabled',true);
                    if(cur_data.cart_btn_choose == '1'){
                        el.find('#shop_cart_2').prop('checked',true);
                    }else{
                        el.find('#shop_cart_1').prop('checked',true);
                    }
                }
        ...
        $(document).on('click',' .choose_sort',function(e){
            e.stopPropagation();
            var xiugai = $(this).attr('xiugai');
            if(xiugai == '1'){
                var parent = $(this).parent().parent();
                var all = parent.find('.is_all').attr('name');
            }
            layui.use('layer', function() { //独立版的layer无需执行这一句
                ...
                layer.open({
                    ...
                    , yes: function (index, layero) { //保存按钮的回调
                        ...
                        var elements1 = ed.find('.sort_group');
                        if(xiugai == 0){
                            ...
                            sort_group.find('.is_all').attr('name',all);
                            parent.replaceWith(sort_group);
                        }
                        var len1 = elements1.length +1;
                        if(len1 >=10){
                            el.find('.add_sort_cat').addClass('hide');
                        }
       ...
       el.on('input propertychange','.sort_group .cus_group_name input',function () {
        ...
        el.on("click",".sort_group .group_item_operation .up_img",function (e) {
                e.stopPropagation();
                var parent = $(this).parent().parent();
                var pre = parent.prev();
        ...
        el.on("click",".sort_group .group_item_operation .down_img",function (e) {
            e.stopPropagation();
            var parent = $(this).parent().parent();
            var pre = parent.next();
        ...
        el.on('click',".group_item_operation .del_img",function (e) {
            ...
            layui.use('layer', function () {
                ...
                layer.alert('<div style="text-align: center;">' + '确认要删除吗？' + '</div>', {
                    ...
                    if(len <=9){
                        el.find('.add_sort_cat').removeClass('hide');
                    }
                    layer.close(ind);
                });
            });
    ...
    */
    ...
    get_data: function(){
        ...
        if(el.find("#shop_goods_name ").prop('checked') == true){
            d.show_name = 0;
        }else{
            d.show_name = 1;
        }
        if(el.find("#shop_goods_price ").prop('checked') == true){
            d.show_price = 0;
        }else{
            d.show_price = 1;
        }
    ...
    get_ajax : function(){
        ...
        util.ajax_json(' /admin/ajax/mp/goods/group', function (response) {
            ...
            if(content.goods_info != undefined){
                ...
                data.market_price = market_price;
                data.goods_len = goods_len;
                m_goods_group.add_group_module_el(manager.get_item_el(data.cur_idx),data,data.if_radius,data.module_style,data.position_style,data.cart_btn,data.cart_btn_choose,data.show_price);
                manager.change_show_module();
            }
        },{data: $.toJSON(data)});
    },
