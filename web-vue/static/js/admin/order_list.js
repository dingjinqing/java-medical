(function () {
    ...
    $('.add_text').on('click', function () {
        ...
        layer.open({
            ...
            yes: function (index) {
                ...
                util.ajax_json("/admin/orders/manage/set/seller/remark", function (d) {
                    if (d && d.error == 0) {
                        layer.msg("保存成功",{
                            time: 2000
                        },function () {
    ...
    $(".btn-deliver").click(function () {
        ...
        util.ajax_json("/admin/orders/manage/has/returning/order?order_sn=" + order_sn, function (d) {
            if (d && d.error == 0) {
    ...
    function part_deliver_dialog(order_sn) {
        layer.open({
            ...
            area: ["800px", "400px"],
            offset:'100px',
            content: "/admin/orders/manage/part/deliver?order_sn=" + order_sn,
            btn: ['发货', '取消'],
            yes: function (index, layero) {
                var body = layer.getChildFrame('body', index);
                var shipping_id = body.find('#shipping_id').val();
                var shipping_no = body.find('#shipping_no').val();
                var cbk_goods = body.find('[name="cbk_goods[]"]:checked');
                var send_num = 0;
                cbk_goods.each(function(){
                    send_num += parseInt($(this).parents('.order-tb-body').find('.send_num').val())
                })
                ...
                /*body.find('input[name="to_send_number"]').each(function (i,v) {
                    if(!parseInt($(this).val())) {
                        util.mobile_alert('请输入发送件数');
                        return false;
                    }
                    if(parseInt(($(this).val()))>$(this).attr('send_number')){
                        util.mobile_alert('发送件数已超出');
                        return false;
                    }
                });*/
                var product_ids = [];
                ...
                body.find('[name="cbk_goods[]"]:checked').each(function () {
                    ...
                    send_numbers.push($(this).parent().parent().find('.send_num').val());
                });
                ...
                $("#send_numbers").val(send_numbers.join(","));
                $("#form1").submit();
            }
        });
    }
    ...
    $(".btn-close").click(function () {
        ...
        layui.use('layer', function () {
            ...
            layer.alert('<div style="text-align: center;">' + '确认要关闭该订单吗?' + '</div>', {
                ...
                $("[name='act']").val("close");
                $("[name='act_order_sn']").val(order_sn);
                $("#form1").submit();
                layer.close(index);
            });
        });
    ...
    $(".btn-star-flag").click(function () {
        ...
        util.ajax_json("/admin/orders/manage/list",function (response) {
            if(response.error == 0){
    ...
    $(".btn-shipping-query").click(function () {
        ...
        util.ajax_json("/admin/orders/manage/express/query", function (d) {
            if (d && d.error == 0) {
    ...
    $(".btn-return").click(function () {
        var order_sn = $(this).parent().attr("order_sn");
        window.location.href = "/admin/orders/manage/return/list?order_sn=" + order_sn;
    });
