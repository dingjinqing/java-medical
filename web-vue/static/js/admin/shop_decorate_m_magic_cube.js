...
var m_magic_cube = {
    ...
    init_ev_table: function (el,data) {
        ...
        this.ev_td_height = $(tbl_el).width() / this.table_size.cols;
        $(tbl_el).find("td").width(this.ev_td_height).height(0);
        $(tbl_el).find("tr").css("height",this.ev_td_height);
    },
    init_ev_block: function (el, block_name) {
        var cube_block = this.select_cube_block[block_name];
        if (!cube_block) return false;
        // this.td_height = this.table_width / this.table_size.cols;
        ...
        if(cube_block.img_url){
            ...
            $(td_el).find("img").width(cube_block.cols * this.ev_td_height).height(cube_block.rows * this.ev_td_height)
            $(td_el).width(cube_block.cols * this.ev_td_height).height(cube_block.rows * this.ev_td_height);
        } else {
            $(td_el).width(cube_block.cols * this.ev_td_height).height(cube_block.rows * this.ev_td_height);
        }
        var tr = td_el.parent();
    ...
    fill_edit_el: function (el, d) {
        ...
        el.find("#select_table_size").change(function () {
            // console.log($(this).val());
            let rows = parseInt($(this).val());
            if (!$.inArray(rows,[4,5,6,7])) rows = 4;
            _this.table_size = {rows:rows,cols:rows};
            _this.select_cube_block = {};
            _this.init_model_table(el,0)
        })
    },
    init_cube_table: function(el,rows,cols) {
        ...
        var _this = this;
        var td_height = this.td_height = this.table_width / cols;
    ...
    init_model_table: function (el,model_id) {
        var _this = this;
        el.find('.change_table_size').hide();
        if (model_id == 1) {
            ...
            el.find('#select_table_size').val(this.table_size.rows);
            el.find('.change_table_size').show();
            _this.init_cube_table(el,this.table_size.rows,this.table_size.cols);
            if (!_this.select_cube_block || !util.count(_this.select_cube_block)) {
                _this.select_cube_block = {};
            }
        }
    ...
    fill_cube_element: function (block_name) {
        ...
        if(this.table_size.rows > 3) {
            if (cube_block.rows > 1 || cube_block.cols > 1) {
                dim +='<span style="word-break: break-all">' + Math.ceil(cube_block.cols * cols_width) + "x" + Math.ceil(cube_block.rows * cols_width) + '</span>像素<br/>或同等比例';
            }
        } else {
        ...
        var td_width = this.td_height = this.table_width / this.table_size.cols;
        td_el.attr("colspan", cube_block.cols);
        td_el.attr("rowspan", cube_block.rows);
        td_el.removeClass("empty");
        td_el.addClass("not-empty").addClass("current");
        td_el.attr("block_name", block_name).css('padding','0px');
        $(td_el).width(cube_block.cols * td_width).height(cube_block.rows * td_width);
    ...
    delete_cube_block: function (block_name) {
        ...
        $(".magic-cube-layout table").find("td[block_name='" + block_name + "']")
            .width(td_width).height(td_width).removeClass("hide").removeClass("not-empty").removeClass("current").addClass("empty")
