    ...
    <form action="/admin/manage/image/list" name="form1" id="form1" method="post">
        ...
        @if ($need_img_width > 0 || $need_img_height > 0)
            <div class="box panel panel-body" style="color:red; line-height:24px;">
                ...
                @if ($need_img_width > 0){{ trans("admin/image.width") }}={{ $need_img_width }}px @endif
                @if ($need_img_height > 0) {{ trans("admin/image.height") }}={{ $need_img_height }}px @endif
                &nbsp;&nbsp;
                <input type='checkbox' id="search_need" name="search_need" @if ($search_need) checked
                       @endif value="1">{{ trans("admin/image.search_needed_image") }}
            </div>
        @endif
                    ...
                    <td style="vertical-align: top;padding:0;">
                        <div class="box ">
                            ...
                            <input name="convert_view_mode" id="convert_view_mode" type="button"
                                   value="@if ($show_type =='list'){{ trans("admin/image.big_show") }}@else{{ trans("admin/image.list_show") }}@endif">

                            {{ trans("admin/image.rows_per_page") }}：
                            <select name="rows_per_page">
                                <option value="30" @if ($rows_per_page== "30") selected="selected" @endif >30</option>
                                <option value="50" @if ($rows_per_page== "50") selected="selected" @endif >50</option>
                                <option value="75" @if ($rows_per_page== "75") selected="selected" @endif >75</option>
                                <option value="100" @if ($rows_per_page== "100") selected="selected" @endif >100
                                </option>
                            </select>
                        ...
                        <div class="box img-list" style="height: 320px;overflow: auto">
                            @if (count($data_list) == 0){{ trans("admin/image.fit_image_not_found") }} @endif

                            <table id="list_data" cellspacing='1' cellpadding='3'
                                   width="100%" @if ($show_type != "list") style="display:none;" @endif>
                                <tr>
                            ...
                            <ul id="bigimg_data" @if ($show_type == "list") style="display:none;" @endif >
                                @foreach ($data_list as $item)
