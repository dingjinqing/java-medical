...
<div class="main-container">
    ...
    <form action="/admin/market/integration/success?top_index=4&id=${request['id']!}" method="post" id="form1">
        {{csrf_field()!}
        {{--        {{print_r($data_list)!}--!}
    ...
    $(document).on('click','.a_click',function () {
        ...
        layui.use('layer', function () { //独立版的layer无需执行这一句
            ...
            layer.open({
                ...
                , area: ['850px','500px']
                , content: '/admin/market/integration/group/detail?id='+ _this.attr('pid')+'&group_id='+  _this.attr('gid')//这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                , btn: ['确定']
                , btnAlign: 'c' //按钮居右
