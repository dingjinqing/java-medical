...
<form name="formData"  action="/admin/orders/activity/order/list?top_index=4&goods_type=${request['goods_type']!}&act_id=${request['act_id']!}&sub_index=${sub_index!}"  method="post" id="form1">
        ...
        <div class="order-list">
            <div class="order-list-table">
                <table width="100%" style="font-size: inherit;">
                    ...
                    <#else>
                    <tr>
                        <#if ($request['goods_type'] != 4 && $request['goods_type'] != 3)
                            <td width="10%">活动名称</td>
                        </#if>
                        <td width="15%">订单号</td>
                        <td width="10%">
                            <#if ($request['goods_type'] == 1) 拼团商品
                            <#elseif>($request['goods_type'] == 3) 砍价商品
                            <#elseif>($request['goods_type'] == 4) 积分兑换商品
                            <#elseif>($request['goods_type'] == 5) 秒杀商品
                            <#elseif>($request['goods_type'] == 7) 降价商品
                            </#if>
                        </td>
                        ...
                        <td width="15%">下单时间</td>
                        <#else>
                        <td width="11.6%">商品数量</td>
                        <td width="11.6%">兑换现金(元)</td>
                        <td width="11.8%">兑换积分数量</td>
                        </#if>
                        <td width="10%">下单人信息</td>
                        <td width="10%">收货人信息</td>
                        <#if ($request['goods_type'] != 3 && $request['goods_type'] != 4)
                            <td width="10%">支付金额</td>
                        </#if>
                        <td width="10%">订单状态</td>
                    </tr>
                    ...
                    <#list ($data_list as $order)
                        ...
                        <#list ($order->order_goods as $i => $order_good)
                           <#if ($request['goods_type'] == 7)
                            <tr class="order-tb-body new_body">
                                <td width="10%">${act_name!}</td>
                                <td width="13%"><a target="_blank" href="/admin/orders/manage/info?order_sn=${order->order_sn!}" style="color: #5a8bff;word-break: break-all">${order->order_sn!}</a></td>
                                <td width="13%">
                                ...
                                <td width="8%">
                                    <#if ($order_good->goods_type == 4)${order_good->goods_price * 100!}积分
                                    <#else>${order_good->market_price!}
                                    </#if>
                                </td>
                                <td width="8%">${order_good->goods_price!}</td>
                                <td width="10%">${order->add_time!}</td>
                                <td width="10%">
                                    <a target="_blank" href="/admin/user/manage/center?top_index=5&user_id=${order->user_id!}&sub_index=0" style="color: #5a8bff">
                                        <p>${order->user_name!}</p>
                            ...
                            <#elseif>($request['goods_type'] == 8)
                               <tr class="order-tb-body">
                                   <td width="10%"><a target="_blank" href="/admin/orders/manage/info?order_sn=${order->order_sn!}" style="color: #5a8bff;word-break: break-all">${order->order_sn!}</a></td>
                                   <td width="13%">
                                       <div class="order-goods-info clearfix">
                                           <div class="fl">
                                               <img src="${order_good->goods_img!}" alt="商品名称"/>
                                   ...
                                   <td>
                                       <#if  ($order_good->group_info->status == 1)
                                           是
                                           <#else>
                                           否
                                       </#if>
                                   ...
                                   <td>
                                       <#if  ($order_good->group_info->is_win_draw == 1)
                                           是
                                       <#else>
                                           否
                                       </#if>
                          ...
                          <#else>
                            <tr class="order-tb-body">
                                ...
                                <td width="15%"><a target="_blank" href="/admin/orders/manage/info?order_sn=${order->order_sn!}" style="color: #5a8bff;word-break: break-all">${order->order_sn!}</a></td>
                                <td width="10%">
                                    <div class="order-goods-info clearfix">
                                        <div class="fl">
                                            <img src="${order_good->goods_img!}" alt="商品名称"/>
                                ...
                                <#if ($request['goods_type']==7)
                                    <td width="10%">${order_good->goods_price!}</td>
                                </#if>
                                <#if ($request['goods_type']!=4)
                                <td width="15%">${order->add_time!}</td>
                                <#else>
                                <td width="8.3%">${order_good->goods_number!}</td>
                                <td width="8.3%">${order_good->goods_price-$order_good->goods_score/100!}</td>
                                <td width="8.3%">${order_good->goods_score!}</td>
                                </#if>
                                <td width="10%">
                                    <a target="_blank" href="/admin/user/manage/center?top_index=5&user_id=${order->user_id!}&sub_index=0" style="color: #5a8bff">
                                        <p>${order->user_name!}</p>
