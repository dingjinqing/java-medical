...
<div class="title">
    <span><a href="/admin/market/view?top_index=4">营销管理</a> / </span><span>分销 / </span><span><a href="/admin/market/distribution/distributer/list?top_index=4">分销员列表</a> / </span><span style="color: #666;">${rebate_user->username!}-已邀请用户列表</span>
</div>
<div class="reserve-container">
    <form action="/admin/market/distribution/distributer/invited/list?top_index=4" id="form1" method="post">
        {{ csrf_field()!}
    ...
    <div class="data">
        <ul>
            <li>累计获得佣金数：<span style="color: red">{{number_format($all_total_money,2)!}</span></li>
        </ul>
    </div>
    <div class="info_table" style="padding-bottom: 50px">
        <table width="100%" style="margin-bottom: 20px">
            ...
            <tr>
                ...
                <th width="10%">注册时间</th>
                <th width="10%">累积返利订单数</th>
                <th width="15%">累积订单返利商品总金额</th>
                <th width="15%">累积返利佣金金额</th>
                <th width="10%">返利有效日期</th>
                <th width="10%">邀请保护日期</th>
            </tr>
            </thead>
            <tbody>
            <#list ($data_list as $item)
            <tr>
                <td width="15%"><a href="/admin/user/manage/center?user_id=${item->user_id!}&top_index=5&sub_index=0" style="color: #0E70CA"  target="_blank">${item->username!}</a></td>
                <td width="15%">${item->mobile!}</td>
                <td width="10%">${item->create_time!}</td>
                <td width="10%">{{intval($item->order_number)!}</td>
                <td width="15%">{{number_format($item->total_can_fanli_money,2)!}</td>
                <td width="15%">{{number_format($item->total_fanli_money,2)!}</td>
                <td width="10%">${item->invite_expiry_date > '2010' ? $item->invite_expiry_date : '永久'!}</td>
                <td width="10%">${item->invite_protect_date >= '2010' ? $item->invite_protect_date : '永久'!}</td>
            </tr>
            </#list>
            </tbody>
        </table>
...
{{--<script type="text/javascript" src="/js/admin/distribution_config.js?v=1.1.7"></script>--!}
