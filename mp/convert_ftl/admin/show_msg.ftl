...
<div class="box panel panel-body">
    <table align="center" width="">
        <tr>
            <th rowspan="2"><img src="http://${image_domain!}/image/admin/info.png" width="50" height="50" border="0"/></th>
            <td><h3 style="font-weight:bolder;margin:0px;padding:0px;">${message['msg_detail']!}</h3></td>
        </tr>
        <tr>
            <td id="navi_hint">
                <span id="hint_sec">30</span>{{ trans("admin/message.jump_link_tips")!}
            </td>
        </tr>
        <tr>
            <td></td>
            <td>
                <ul style="margin:0px;padding:14px;border-top:1px solid #e4e4e4;">
                    <#if ($message['links'])
                        <#list ((array)$message['links'] as $link)
                    ...
                    <#else>
                        <li style="list-style-type:lower-latin;">
                            <a href="javascript:history.go(-1)" onclick="javascript:history.go(-1);">{{ trans("admin/message.return_last_page")!}</a>
                        </li>
                    </#if>
                </ul>
