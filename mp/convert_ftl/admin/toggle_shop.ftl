        ...
        <div class="main-top">
            ...
            <div class="main-ul">
                <#if ($data_list != "")
                    ...
                    <li class="shop-li" is_enabled="${item->is_enabled!}" business_state="${item->business_state!}" expire_time_status="${item->expire_time_status!}">
                        <a <#if ($item->is_enabled==0) href="/admin/account/shop/switch?shop_id=${item->shop_id!}" <#else> style="cursor: default;" </#if>>
                            <div class="shop-state" style="<#if ($item->business_state==1 && $item->expire_time_status == 0 &&  $item->is_enabled == 0) background: #fff;<#else> background:#fef4e8;</#if>">
                                <#if ($item->is_enabled == 1)
                                    <img src="http://${image_domain!}/image/admin/no_use.png" alt="" class="sel-top-img">
                                    {{--<span class="no-use-label" style="padding-right: 7%;position: relative;">已禁用<div class="line"></div>--!}
                                    {{--</span>--!}
                                    <span class="no-use-label">已禁用</span>
                                <#elseif>($item->business_state!=1)
                            ...
                            <p>有效期：{{ substr($item->created,0,10)!} ~ {{ substr($item->expire_time,0,10)!}</p>
...
<div id="message" message="{{session("message")!}"></div>
