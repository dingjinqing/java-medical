        ...
        <div class="goods-box">
            ...
            <div class="goods-box-edit">
                <div class="goods-edit-basic">
                    <h2 class="h2_title">${returnTypeName[$return_order->return_type]!}申请</h2>
                    <div class="refund_status_desc" onload="show_time()">
                        <#if ($return_order->return_type == 1)
                            <#if ( $return_order->refund_status == 1)
                                <div style="display: inline-block">
                                    ...
                                    <#if ($auto_return && ($return_order->apply_time > $auto_return_time))
                            ...
                            <#elseif>($return_order->refund_status == 4)
                                <div style="display: inline-block">
                                    ...
                                    <#if ($auto_return && ($return_order->shipping_or_refund_time > $auto_return_time))
                        ...
                        <#elseif>(in_array($return_order->return_type,[0,2,3]))
                            <#if ($return_order->refund_status == 4)
                                ...
                                <div style="display: inline-block" class="btn_box">
                                    <div class="btn-1 btn-common btn-pass" style="cursor:pointer;">同意买家${returnTypeName[$return_order->return_type]!}</div>
                                    <div class="btn-2 btn-close btn-not-pass" style="cursor:pointer;">拒绝${returnTypeName[$return_order->return_type]!}申请</div>
                                </div>
                            <#elseif>( $return_order->refund_status == 5)
                                <h3>商家同意${returnTypeName[$return_order->return_type]!}，退款成功</h3>
                            <#elseif>($return_order->refund_status == 6)
                                <h3>商家拒绝${returnTypeName[$return_order->return_type]!}申请</h3>
                            <#elseif>( $return_order->refund_status == 7 && $return_order->operator == 1)
                                <h3>买家主动撤销${returnTypeName[$return_order->return_type]!}申请</h3>
                            <#elseif>($return_order->refund_status == 7 && $return_order->operator == 2)
                                <h3>撤销退款申请（系统自动处理）</h3>
                                <div>买家未在商家审核退款申请之后7日内，提交退货物流（或商家未确认收货），退款申请已自动撤销</div>
                            </#if>
            ...
            <div class="goods-box-edit">
                <div class="goods-edit-basic">
                    <h2 class="h2_title">${returnTypeName[$return_order->return_type]!}申请详情</h2>
                    <table class="order-basic-table table table-bordered">
                        ...
                        <tr>
                            ...
                            <td colspan="3"><strong>${returnTypeName[$return_order->return_type]!}</strong></td>
                        </tr>
                        <tr>
                                ...
                                <#if ($return_order->refund_status == 4 || $return_order->refund_status == 2 )
                                    <div class="refund-info" style="line-height: 24px">
                                        ...
                                        <div style="color: #666;">
                                            ...
                                            <#if ($order->bk_order_money > 0)退尾款：￥<span class="text-warning refund-bk-order-money">0.00</span></#if>
                                    ...
                                    <p style="color:#666">
                                        注：总退款金额 = 退商品金额+退运费金额，扣款优先级：<#if ($order->bk_order_money > 0)定金尾款，</#if>会员卡余额，余额，积分，支付金额
                                    </p>
                                <#else>
                                    <strong>
                                        <p>
                                        ...
                                        <#if ($refund_detail)
                                            <p>
                                                退款详情：
                                                <#if ($refund_detail['bk_order_money'] > 0)退尾款：<span
                                                        class="text-warning">￥{{ number_format($refund_detail['bk_order_money'],2,".","")!}</span> </#if>
                ...
                <#if  ($order->order_pay_way == 2)
                    <div class="goods-edit-basic" id="insteadPay">
                        <h2 class="h2_title">代付退款明细</h2>
                        <table class="order-basic-table table table-bordered">
                            <thead>
                            <tr>
                                <th width="20%">支付流水号</th>
                                <th width="20%">支付时间</th>
                                <th width="10%">付款人</th>
                                <th width="10%">代付金额</th>
                                <th width="10%">退款金额</th>
                            ...
                            <tbody id="instead_order_list" style="text-align: center;"></tbody>
                        </table>
                ...
                <div class="goods-edit-basic">
                    ...
                    <#list ($change_list as $change)
                        ...
                        <div class="recode">
                            ...
                            <div class="bottom">
                                <ul>
                                    <li>退款类型：${returnTypeName[$return_order->return_type]!}</li>
    ...
    function loadInsteadOrderList(page) {
        util.ajax_json('/admin/orders/suborder/list', function (response) {
            if (response.error == 0) {
                ...
                console.log(list);
                if (list.length <= 0) $('#insteadPay').hide();
                for (var i in list) {
                    var instead_order_template = $('#instead_order_template table tbody').clone();
                    instead_order_template.find('td').eq(0).html(list[i].pay_sn);
                    instead_order_template.find('td').eq(1).html(list[i].pay_time);
                    instead_order_template.find('td').eq(2).html(list[i].username);
                    instead_order_template.find('td').eq(3).html(list[i].money_paid);
                    instead_order_template.find('td').eq(4).html(list[i].refund_amount);
                    instead_order_template.find('td').eq(5).html(list[i].refund_time);
                    html += instead_order_template.html();
                }
                $('#instead_order_list').html(html);
            }
        }, {refund_detail:1, ret_id: "${_GET['ret_id']!}", page:page});
    }
    <#if  ($order->order_pay_way == 2)
        loadInsteadOrderList(1);
    </#if>
</script>
<#include "/admin/footer.ftl">
<script src="{{asset('js/admin/order_return.js')!}?v=1.0.6" type="text/javascript"></script>
