        ...
        <div class="nav-role">
            <ul id="tab" class="nav-child-tabs">
                <li <#if ($request['nav'] == 0) class="active" </#if>>
                    <a href="/system/market/integration/list?nav=0&shop_id=${shop_id!}">全部瓜分积分活动</a>
                </li>
                <li <#if ($request['nav'] == 1) class="active" </#if>>
                    <a href="/system/market/integration/list?nav=1&shop_id=${shop_id!}">进行中</a>
                </li>
                <li <#if ($request['nav'] == 2) class="active" </#if>>
                    <a href="/system/market/integration/list?nav=2&shop_id=${shop_id!}">未开始</a>
                </li>
                <li <#if ($request['nav'] == 3) class="active" </#if>>
                    <a href="/system/market/integration/list?nav=3&shop_id=${shop_id!}">已过期</a>
                </li>
                <li <#if ($request['nav'] == 4) class="active" </#if>>
                    <a href="/system/market/integration/list?nav=4&shop_id=${shop_id!}">已停用</a>
                </li>
            </ul>
        </div>
        {{--<div class="btn_adds">--!}
            {{--<a href="/system/market/integration/add" target="_blank">添加瓜分积分活动</a>--!}
        {{--</div>--!}
    </div>

    <div class="main-container" style="padding-top: 0px">
        <form action="/system/market/integration/list?nav=${request['nav']!}" method="post" id="form1" name="form1">
            {{ csrf_field()!}
                                ...
                                <tr>
                                    ...
                                    {{--<td class="tb-decorate-a">--!}
                                        {{--<#if  (in_array($item->pin_integration_status, [1, 2]))--!}
                                            {{--<a href="/system/market/integration/add?id=${item->id!}" target="_blank">--!}
                                                {{--编辑--!}
                                            {{--</a><br/>--!}
                                            {{--<div class="erweima">--!}
                                        ...
                                        {{--<#if  ($item->sec_kill_status != 2)--!}
                                            {{--<a href="/system/market/integration/detail?id=${item->id!}">--!}
                                              {{--参团明细--!}
                                            {{--</a><br/>--!}
                                            {{--<a href="/system/market/integration/success?id=${item->id!}">--!}
                                              {{--成团明细--!}
                                            {{--</a><br/>--!}
                                        {{--</#if>--!}
        ...
        if (action == 'or_enable') {
            ...
            layui.use('layer', function () {
                ...
                layer.alert('<div style="text-align: center;">' + lay_message + '</div>', {
                    ...
                    util.ajax_json('/system/market/integration/list', function (response) {
                        if (response.error == 0) {
                            $("#form1").submit();
                        } else {
