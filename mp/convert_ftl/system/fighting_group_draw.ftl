...
<link rel="stylesheet" href="/css/system/fighting_group_draw.css?v=1.0.1" type="text/css" />
<style>
    td a{
        color: #5A8BFF;
        ...
        <div class="nav-role">
            <ul id="tab" class="nav-child-tabs">
                <li <#if (empty($nav))class="active"</#if>>
                    <a href="/system/market/groupdraw/list?top_index=4&shop_id=${shop_id!}">全部拼团抽奖活动</a>
                </li>
                <li <#if ($nav == 1)class="active"</#if>>
                    <a href="/system/market/groupdraw/list?nav=1&top_index=4&shop_id=${shop_id!}" >进行中</a>
                </li>
                <li <#if ($nav == 2)class="active"</#if>>
                    <a href="/system/market/groupdraw/list?nav=2&top_index=4&shop_id=${shop_id!}" >未开始</a>
                </li>
                <li <#if ($nav == 3)class="active"</#if>>
                    <a href="/system/market/groupdraw/list?nav=3&top_index=4&shop_id=${shop_id!}" >已过期</a>
                </li>
                <li <#if ($nav == 4)class="active"</#if>>
                    <a href="/system/market/groupdraw/list?nav=4&top_index=4&shop_id=${shop_id!}">已停用</a>
                </li>
            </ul>
        </div>
        <form name="formData" action="/system/market/groupdraw/list?nav=${request['nav']!}" id="form1" method="post">
            {{ csrf_field()!}
            ...
            {{--<ul class="add-child-ul">--!}
                {{--<li>--!}
                    {{--<a class="add-child-btn" href="/system/market/groupdraw/add?top_index=4&nav=5" target="_blank">添加拼团抽奖活动</a>--!}
                {{--</li>--!}
            {{--</ul>--!}
                            ...
                            <tbody>
                                <#list ($data_list as $item)
                                    <tr>
                                        ...
                                        {{--<td item="${item->id!}">--!}
                                            {{--<#if  (in_array($nav, [1, 2]))--!}
                                                {{--<a href="/system/market/groupdraw/add?top_index=4&id=${item->id!}">编辑</a>--!}
                                            {{--</#if>--!}
                                            ...
                                            {{--<#if ($item->start_time < date("Y-m-d H:i:s",time()))--!}
                                                {{--<a href="/system/orders/activity/order/list?top_index=${top_index!}&sub_index=${sub_index!}&act_id=${item->id!}&goods_type=8"  target="_blank">查看活动订单</a>--!}
                                                {{--<a href="/system/market/groupdraw/user?top_index=4&id=${item->id!}"  target="_blank">查看参与用户</a>--!}
                                                {{--<a href="/system/user/source/detail?top_index=${top_index!}&sub_index=${sub_index!}&act_id=${item->id!}&source=group_draw"  target="_blank">获取新用户明细</a>--!}
                                                {{--<a href="/system/market/groupdraw/detail?top_index=4&id=${item->id!}"  target="_blank">开团明细</a>--!}
                                            {{--</#if>--!}
     ...
     $('.abort').click(function(){
         ...
         layui.use('layer', function () {
             ...
             layer.alert('<div style="text-align: center;">' + '确认要停用吗？' + '</div>', {
                 ...
                 util.ajax_json("/system/market/groupdraw/list",function (res) {
                     if(res.error == 0){
