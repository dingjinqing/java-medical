...
<form action="/system/shop/list" name="form1" id="form1" method="post">
    ...
    <div class="box panel ">
        <div class="panel-body">
            {{--账户：<input type="text" name="sys_id" value="${sys_id!}" size="6" placeholder="输入账户ID进行查询" style="width: 15%">--!}
            {{--<input type="text" name="company" value="${company!}" size="6" placeholder="输入账号公司名进行查询" style="width: 15%">--!}
            <input type="text" name="account_key" value="${request['account_key']!}" size="6" placeholder="账号ID、公司" style="width: 220px">
            <span class="text-info" style="margin-right: 20px">${account ? $account->user_name:""!}</span>
            <input type="text" style="width: 250px" name="keywords" placeholder="店铺ID、店铺名称、手机号、小程序名称" value="${request['keywords']!}">
            ...
            <select name="shop_flag" id="" class="form-control">
                ...
                <#list ($shop_flag_list as $k=>$flag)
                <option value="${k!}" <#if ($request['shop_flag'] == $k && !is_null($request['shop_flag'])) selected </#if>>${flag!}</option>
                </#list>
            </select>
        ...
        <div class="panel-body">
            ...
            <select name="hid_bottom" id="" class="form-control" >
                ...
                <option value="1" <#if ($request['hid_bottom'] == 1) selected </#if>>隐藏</option>
                <option value="0" <#if ($request['hid_bottom'] == 0 && !is_null($request['hid_bottom'])) selected </#if>>显示</option>
            </select>
            <div class="search" >{{ trans("system/common.operation.search")!}</div>
        </div>
    </div>
                ...
                <#list ($data_list as $item)
                    <tr style="text-align:center;">
                        ...
                        <td class="expire_time" expire_time="${item->expire_time!}"><#if ($item->expire_time != "")${item->expire_time!}</#if>
                            ...
                            <br><#if ($item->expire_time > date('Y-m-d H:i:s',time()))(使用中)<#elseif>($item->expire_time < date('Y-m-d H:i:s',time()))(已过期)</#if></td>
{{--                        <td>${item->is_enabled==1?"是":"否"!}</td>--!}
                        <td><a href="#" class="is_enabled" shop_id="${item->shop_id!}" style="color: #86a7cb;">${item->is_enabled==1?"已禁止":"未禁止"!}</a></td>
                        <td>${item->app_id? ($item->is_auth_ok==1?"已授权":"已取消授权") : "未绑定授权"!}</td>
                        ...
                        {{--</td>--!}
                        <td>
                            <#if (empty($item->shop_flag))店+<#else>${shop_flag_list[$item->shop_flag]!}</#if></td>
                        <td><a href="#" class="hid_bottom" shop_id="${item->shop_id!}" style="color: #86a7cb;">${item->hid_bottom==1?"隐藏":"显示"!}</a></td>
                        <td>
                            <#if ( $item->app_id)
