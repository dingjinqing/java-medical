...
<ul id="tab" class="nav nav-tabs">
    ...
    <li <#if ($nav_type==0)class="active"</#if>>
        <a href="#" data-toggle="tab" onclick="location.href = '/system/shop/mp/jump/list';">申请发布列表</a>
    </li>
</ul>

<form action="/system/shop/mp/jump/list" name="form1" id="form1" method="post">
    ...
    <div class="box panel ">
        <div class="panel-body">
            ...
            <select name="flag" id="" class="form-control">
                <option value="-1" selected>选择发布状态</option>
                <option value="0" <#if (!is_null($flag) && $flag == 0) selected </#if>>未发布</option>
                <option value="1" <#if ($flag == 1) selected </#if>>已发布</option>
            </select>
            ...
            <table cellspacing='1' cellpadding='3' width="100%" class="table" id="main-table">
                ...
                <#list ($data_list as $item)
                    <tr style="text-align:center;">
                        ...
                        <td class="shop_names" title="${item->principal_name!}">${item->nick_name!}</td>
                        ...
                        <td>${item->add_time!}</td>
                        <td><#if ($item->flag == 0)未发布<#elseif>($item->flag == 1)已发布</#if></td>
