...
(function () {
            ...
            switch (n) {
                case "first_num":
                {
                    ...
                    if (isNaN(t) || !isNaN(t) && t <= 0) {
                        util.mobile_alert("首件件数必须为大于0的整数");
                        el.focus();
                        return false;
                    }
                    break;
                ...
                case "first_fee":
                {
                    ...
                    if (isNaN(t) || !isNaN(t) && t < 0) {
                        util.mobile_alert("首件运费必须为大于0的数");
                        el.focus();
                        return false;
                    }
                    break;
                ...
                case "continue_fee":
                {
                    var t = parseFloat(el.val());
                    if (isNaN(t) || !isNaN(t) && t < 0) {
                        util.mobile_alert("续件运费必须为大于0的数");
                        el.focus();
                        return false;
                    }
                    break;
            ...
            for (var i = 0; i < tr_list.length; i++) {
                switch ($(tr_list[i]).find(".fee-0-condition").val()) {
                    ...
                    case "2":
                    {
                        ...
                        var t = parseFloat(el.val());
                        if (isNaN(t) || !isNaN(t) && t < 0) {
                            util.mobile_alert("金额必须为大于等于0的整数");
                            el.focus();
                            return false;
                        }
                        break;
    ...
    $(".btn-save").click(function (e) {
        ...
        if (!check_is_valid_template())
            return false;
        var r = /^\+?[1-9][0-9]*$/;
        if( ! r.test($(".def_fee_template .first_num").val())){
            util.mobile_alert("请输入整数");
            return false;
        }
        if( ! r.test($(".def_fee_template .continue_num").val())){
            util.mobile_alert("请输入整数");
            return false;
        }
        var first_num = parseInt($(".def_fee_template .first_num").val());
        ...
        var continue_num = parseInt($(".def_fee_template .continue_num").val());
        ...
        if (has_fee_0_condition) {
            $("#deliver_fee_0_tbl tr.tr-fee-0-area").each(function () {
                ...
                var fee_temp = {
                    ...
                    fee_0_con1_num: parseInt($(this).find(".fee_0_con1_num").val()),
                    fee_0_con2_fee: parseFloat($(this).find(".fee_0_con2_fee").val()),
                    fee_0_con3_num: parseInt($(this).find(".fee_0_con3_num").val()),
