...
var util = {
    ...
    init_ajaxfileupload: function (upload_btn_id, file_el_id, cb, formData, accept, multiple, fileSizeLimit) {
        ...
        var action = '/admin/ajax/image/upload';
    ...
    init_ajax_video_upload: function (upload_btn_id, file_el_id, cb, formData, accept, multiple, fileSizeLimit) {
        ...
        var button = $('#' + upload_btn_id);
        // var url = location.href;
        var action = '/admin/manage/video/upload';
     ...
     * @param limit     每页显示条数
     */
    loadPage: function(total, currPage, callback, elem, limit) {
        elem = arguments[3] == undefined ? 'test1' : elem;
        limit = arguments[4] == undefined ? 20 : limit;
        function pageInit() {
            laypage.render({
                elem: elem,
                count: total,
    ...
    set_line_value:function(type,num) {
        if(num == '' || num == 0) num = 1;
        for( var j=0;j<num;j++){
            var value = $('.set_line_value').find("."+type).eq(0).find("input").eq(j).val();
            $('.set_line_value').each(function (i,v) {
                $('.set_line_value').find("."+type).eq(i).find("input").eq(j).val(value);
            })
        }
}
};
