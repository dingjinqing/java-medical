...
function init_drag_event() {
    ...
    $("#save_content").click(function (event) {
        ...
        layui.use('layer', function () {
            ...
            layer.alert('<div style="text-align: center;">' + '页面将自动保存为草稿并发布到线上' + '</div>', {
                title: ['页面发布提醒', 'text-align:center;padding: 0px;']
                , area: '260px'
                , closeBtn: 0
                , btn: ['确定发布', '取消发布']
            }, function (index) {
    ...
    $(document).on("mouseover", "#drag_area div.row_item", function (e) {
        if($(this).attr('module_name') == 'm_video'){
            var videoId = $(this).find('video').attr('id');
            var video = document.getElementById(videoId);
            video.addEventListener('play',function(){
                $(this).parent().parent().find('.video_title').hide();
            });
            video.addEventListener('pause',function(){
                $(this).parent().parent().find('.video_title').show();
            })
        }
...
var manager = {
    ...
    add_m: function (index, module_name, data) {
        ...
        if (data == undefined) {
            ...
            if(module_name == 'm_video'){
                el.find('.video_module video').attr('id','video'+ this.cur_idx);
            }
    ...
    save_all: function (state) {
        ...
        $("#drag_area div.row_item").each(function () {
            ...
            if(t["c_" + cur_idx].module_name=='m_text_image'){
                k++;
            }
            ...
            if (page_id > 0) {
                if (self.num >= self.use || self.num < 0) {
                    util.ajax_json('/admin/manage/decorate/update', function (res) {
                        console.log(res)
                        if (res && res.error == 0) {
                            if (param.page_state == 3) {
                                util.mobile_alert('回退成功');
                            ...
                            if (!res.content) {
                                ...
                                layui.use('layer', function () { //独立版的layer无需执行这一句
                                    ...
                                    layer.open({
                                        type: 1
                                        , title: ['手机扫码预览', 'text-align:center;padding: 0px;']
                                        , offset: 'auto' //具体配置参考：http://www.layui.com/doc/modules/layer.html#offset
                                        , id: 'preview1' //防止重复弹出
                                        , content: '<div style="padding: 34px 52px;text-align: center"><img style="width:46px;height: 46px;margin-bottom: 20px" src="/image/admin/icon_warn.png"><p><a href="/wechat/no/authorization?top_index=1">小程序未授权,暂不支持预览，点击授权小程序</a></p></div>'
                                        , shade: 0 //不显示遮罩
                                    });
                                })
                            }
...
//                    this.add_m(-1, 'm_shop');
    ...
    check_module_data: function (cur_data) {
        if (cur_data.module_name == 'm_goods') {
            ...
            if (cur_data.img_items.length == 0) {
                util.mobile_alert('请添加轮播图片');
                return false;
            }
            ...
            for( var i in cur_data.nav_group){
                if ( !cur_data.nav_group[i].nav_src) {
                    util.mobile_alert('请上传图片导航模块图片');
                    return false;
                }
            }
            ...
            if(cur_data.video_poster == 2){
                if(cur_data.img_url == '' || cur_data.img_url == undefined){
                    util.mobile_alert('请上传自定义封面');
                    return false;
                }
            }
        ...
        } else if(cur_data.module_name == 'm_text_image'){
            if (!cur_data.img_url) {
                util.mobile_alert('请上传图片');
                return false;
            }
            if (cur_data.rich_text == ""){
                util.mobile_alert('请输入文本');
                return false;
            }
        }else if(cur_data.module_name == 'm_shop_announce'){
           if(!cur_data.shop_text){
               util.mobile_alert('请输入文本');
               return false;
           }
        } else if (cur_data.module_name == 'm_group_draw') {
            if (!cur_data.group_draw_id) {
