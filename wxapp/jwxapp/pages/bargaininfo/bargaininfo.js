...
global.wxPage({
  ...
  toItem: function () {
    var bargain_id = bargain_info.record_info.bargain_id;
    util.reLaunch({
  ...
  toCheckout: function (e) {
    ...
    var record_id = bargain_info.record_info.id;
    if (bargain_info.record_info.bargain_type == 1 && bargain_info.record_info.is_ordered == 1) {
      var check_money = parseFloat(bargain_info.record_info.goods_price - bargain_info.record_info.bargain_money).toFixed(2);
  ...
  to_buy: function () {
    util.navigateTo({
      url: "/pages/item/item?goods_id=" + bargain_info.record_info.goods_id
    })
  },
  toWhere: function (e) {
    var bargain_id = bargain_info.record_info.bargain_id;
    if (bargain_info.record_info.prd_desc != "") {
      util.reLaunch({
        url: '/pages/bargainitem/bargainitem?bargain_id=' + bargain_id,
      })
    } else if (bargain_info.record_info.prd_desc == "") {
      var choose_list = {};
      choose_list.user_id = util.getCache('user_id');
      choose_list.goods_id = bargain_info.record_info.goods_id;
      choose_list.prd_id = bargain_info.record_info.prd_id;
      choose_list.bargain_id = bargain_info.record_info.bargain_id;
      choose_list.goods_price = bargain_info.record_info.goods_price;
  ...
  toKnajia: function (e) {
    ...
    var open_id = util.getCache("openid");
    var form_id = e.detail.formId;
    if (util.getCache('mobile') == '' && bargain_info.record_info.need_bind_mobile == 1) {
...
function request_kanjia(that) {
  util.api("/api/wxapp/bargain/info", function (res) {
    if (res.error != 0) {
      ...
      if (bargain_info.record_info.goods_id) {
        util.getUserLocation(function (loc) {
          util.api('/api/wxapp/user_goods/record', function (res1) { }, {
            goods_id: bargain_info.record_info.goods_id, active_id: bargain_info.record_info.bargain_id, active_type: 3, type: 1, lat: loc.latitude || '',
            lng: loc.longitude || ''
          })
        })
      }
      if (bargain_info.record_status.status == 1 || bargain_info.record_status.status == 2) {
        util.showModal('提示', bargain_info.record_status.msg, function () {
          wx.navigateBack()
        });
      }
      if (bargain_info.record_info.expectation_price == null) {
        bargain_info.record_info.expectation_price = 0.00
      }
      if (bargain_info.record_info.floor_price == null) {
        bargain_info.record_info.floor_price = 0.00
      }
      if (bargain_info.record_info.bargain_type == 1) {
        var left_value = (parseFloat(bargain_info.record_info.goods_price) - parseFloat(bargain_info.record_info.expectation_price)) * 670 / (parseFloat(bargain_info.record_info.goods_price) - parseFloat(bargain_info.record_info.floor_price));
        bargain_info.record_info.min_bargain_money = parseFloat(bargain_info.record_info.goods_price - bargain_info.record_info.floor_price).toFixed(2);
        that.setData({
          left_value: parseFloat(left_value)
        })
      }
      if (!bargain_info.record_info.user_avatar) {
        bargain_info.record_info.user_avatar = imageUrl + "/image/admin/head_icon.png";
      }
      //剩余未砍价的钱
      bargain_info.record_info.not_freee_monet = parseFloat(bargain_info.record_info.goods_price -
        bargain_info.record_info.bargain_money - bargain_info.record_info.expectation_price).toFixed(2);

      //进度条
      if (bargain_info.record_info.bargain_type == 0) {
        var bili = bargain_info.record_info.bargain_money /
          (bargain_info.record_info.goods_price - bargain_info.record_info.expectation_price);
      } else {
        var bili = bargain_info.record_info.bargain_money /
          (bargain_info.record_info.goods_price - bargain_info.record_info.floor_price);
      }
      bargain_info.record_info.progress_present = parseFloat(bili).toFixed(2);
      bargain_info.record_info.progress_present = bargain_info.record_info.progress_present * 100;
      //倒计时
      that.data.total_micro_second = bargain_info.record_info.remaining_time;
      that.countdown();

      // 进度条显示已砍价金额
      money_now_left = parseFloat(bargain_info.record_info.progress_present / 100) * 670;
