...
global.wxComponent({
  ...
  properties: {
    ...
    default_prd_spec: {
      ...
      observer(newVal, oldVal, changedPath) {
        if (newVal) {
          // this.setData({ default_prd_spec: newVal });
          // console.log(newVal)
          this.data.default_prd_spec = newVal;
    ...
    default_goods_number: {
      ...
      observer(newVal, oldVal, changedPath) {
        if (newVal > 0) {
          // this.setData({ default_goods_number: newVal });
          // console.log(newVal)
          this.data.default_goods_number = newVal;
          // this.defaultSelectSpec();
        }
      }
    },
  ...
  ready() {
    ...
    if (dd == false) {
      util.api("/api/wxapp/pledge", function(res) {
        ...
        goods_id:that.data.gd.goods_id,
        brand_id:that.data.gd.brand_id,
        sort_id:that.data.gd.sort_id
      })
    }
  ...
  methods: {
    ...
    init() {
      var d = {
        ...
        is_max: false,
        goods_price: '0.00',
        goods_number: this.data.default_goods_number || 1,
        spec_list: {},
        select_prd: null,
        select_specs: {},
        unselect_spec_names: '',
    ...
    getGoodsPrice() {
      ...
      this.data.market_price = d.prd_market_price || d.market_price;
    ...
    getDefaultSpec() {
      this.initSpecList();
      var select_prd_specs = this.data.default_prd_spec;
      if (select_prd_specs) return select_prd_specs;
      var prd_list = this.data.gd.specs.prd_list;
      for (var prd_specs in prd_list) {
        if (prd_list[prd_specs][this.stockField()] > 0) {
          return prd_specs;
        }
      }
      return false;
