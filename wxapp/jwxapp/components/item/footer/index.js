  ...
  methods: {
    ...
    async actCheckOut () {
      ...
      if (this.data.activity && this.data.activity.activityType === 10) {
        params.preSaleInfo = {
          ...this.data.activity,
          preSalePrdInfo: this.data.productInfo.actProduct
        }
    ...
    getGroupPirce (type) {
      ...
      if (type === 'grouper') {
        realPriceArr = this.data.activity.groupBuyPrdMpVos.map(item => {
          return item.grouperPrice
        })
      } else {
        realPriceArr = this.data.activity.groupBuyPrdMpVos.map(item => {
          return item.groupPrice
        })
      }
    ...
    getProducesMinPrice () {
      ...
      let minPrice = this.getMin(realPriceArr),
        maxPrice = this.getMax(realPriceArr)
    ...
    setButtonStyle () {
      ...
      } else if (activity && [1, 3, 4, 5, 8, 10].includes(activity.activityType) && [1, 2, 3, 4, 5, 6].includes(activity.actState) && (this.data.position === 'footer' || this.data.triggerButton !== 'left')) {
