   ...
   */
	...
	requsetMessage() {
		...
		if (
			...
			this.data.targetUserInfo.sessionStatus === 4
		)
			this.timer = setInterval(this.messageApi, 5000)
	},
	messageApi() {
		util.api(
			'/api/wxapp/im/session/pull',
			res => {
				console.log(res)
	...
	createPrescription() {
		util.jumpLink(
			`pages2/prescribe/prescribe${util.getUrlParams({
	...
	chatEnd() {
		util.showModal(
			...
			() => {
				util.api(
					'/api/wxapp/inquiry/order/status/update',
					async res => {
	...
	handleShowPrescriptionDialog(e) {
		...
		util.api(
			'/api/wxapp/prescription/details',
			res => {
				if (res.error === 0) {
					this.setData({
