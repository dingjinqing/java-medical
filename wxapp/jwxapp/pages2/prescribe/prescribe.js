...
global.wxPage({
  ...
  onLoad: function (options) {
    let { patientId, userId: patientUserId } = options,
  ...
  requestGoodsList() {
    util.api(
      '/api/wxapp/medical/goods/page/list',
      (res) => {
        console.log(res)
        if (res.error === 0) {
          if (this.data.dialogPageParams.currentPage === 1) {
            this.setData({
              dialogGoodsList: [
                [
                  ...this.resetDialogGoodsList(
                    res.content.dataList,
                    parseInt(this.data.dialogPageParams.currentPage) - 1
                  ),
                ],
              ],
            })
  ...
  resetDialogGoodsList(dataList, parentIndex) {
    return dataList.map((item) => {
      ...
      item.cartNumber = this.getDefaultGoodsNum(
        item.goodsId,
        this.data.goodsIdNum
      )
  ...
  toggleSelect(e) {
    ...
    let cacheGoodsIdList = this.data.cacheGoodsIdList,
      cachePageGoodsList = this.data.cachePageGoodsList,
      cacheGoodsIdNum = this.data.cacheGoodsIdNum
    if (selected) {
      cacheGoodsIdList.push(Target.goodsId)
      cachePageGoodsList.push(Target)
      cacheGoodsIdNum.push({
        goodsId: Target.goodsId,
        dragSumNum: Target.cartNumber,
        prdId: Target.goodsSpecProducts[0].prdId,
      })
      this.setData({
        cacheGoodsIdList,
        cachePageGoodsList,
      ...
      cacheGoodsIdList.splice(
        cacheGoodsIdList.findIndex((item) => item === Target.goodsId),
        1
      )
      cachePageGoodsList.splice(
        cachePageGoodsList.findIndex((item) => item.goodsId === Target.goodsId),
        1
      )
      cacheGoodsIdNum.splice(
        cacheGoodsIdNum.findIndex((item) => item.goodsId === Target.goodsId),
        1
      )
      this.setData({
        cacheGoodsIdList,
  ...
  delGoodsItem(e) {
    ...
    let goodsIdList = this.data.goodsIdList,
      pageGoodsList = this.data.pageGoodsList,
      goodsIdNum = this.data.goodsIdNum
    goodsIdList.splice(
      goodsIdList.findIndex((item) => item === goodsId),
      1
    )
    pageGoodsList.splice(
      pageGoodsList.findIndex((item) => item.goodsId === goodsId),
      1
    )
    goodsIdNum.splice(
      goodsIdNum.findIndex((item) => item.goodsId === goodsId),
      1
    )
    this.setData({
      goodsIdList,
  ...
  getPatientDetail() {
    util.api(
      '/api/wxapp/user/patient/get/detail',
      (res) => {
        if (res.error === 0) {
  ...
  cartNumChange({ detail: goodsInfo }) {
    ...
    let targetIndex = this.data.dialogGoodsList[
      goodsInfo.parentIndex
    ].findIndex((item) => item.goodsId === goodsInfo.goodsId)
    let cachePageGoodsList = this.data.cachePageGoodsList,
      cacheGoodsIdNum = this.data.cacheGoodsIdNum
    let cacheGoodsListIndex = cachePageGoodsList.findIndex(
      (item) => item.goodsId === goodsInfo.goodsId
    )
    let cacheGoodsIdNumIndex = cacheGoodsIdNum.findIndex(
      (item) => item.goodsId === goodsInfo.goodsId
    )
    this.setData({
      [`dialogGoodsList[${goodsInfo.parentIndex}][${targetIndex}].cartNumber`]:
        goodsInfo.type === 'plus'
          ? goodsInfo.cartNumber + 1
          : goodsInfo.cartNumber - 1,
    })
    if (cacheGoodsListIndex !== -1 && cacheGoodsIdNumIndex !== -1) {
      this.setData({
        [`cachePageGoodsList[${cacheGoodsListIndex}].cartNumber`]:
          goodsInfo.type === 'plus'
            ? goodsInfo.cartNumber + 1
            : goodsInfo.cartNumber - 1,
        [`cacheGoodsIdNum[${cacheGoodsIdNumIndex}].dragSumNum`]:
          goodsInfo.type === 'plus'
            ? goodsInfo.cartNumber + 1
            : goodsInfo.cartNumber - 1,
      })
