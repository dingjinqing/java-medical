...
const pageConfig = {
  'pages1': [
    ...
    "distributionspread/distributionspread",
    "distrigrade/distrigrade",
    "distrirecord/distrirecord",
    "form/form",
    "integral/integral",
    ...
    "pinintegrationdetail/pinintegrationdetail",
    "pinlotteryinfo/pinlotteryinfo",
    "pinlotteryitem/pinlotteryitem",
    "pinlotterylist/pinlotterylist",
    "scancode/scancode",
...
var nav = {
  ...
  jumpLink(url, linkType, success, fail, complete) {
    if (cache.getCache('isPageJumping') == 1) {
      console.log('has jumping!'); return false;
    }
    cache.setCache('isPageJumping',1)
    ...
    o.complete = function (res) {
      ...
      cache.removeCache('isPageJumping')
    }
  ...
  validUrl(url) {
    ...
    for (var root in pageConfig) {
      if (arr[1] != root && pageConfig[root].indexOf(arr[2] + "/" + arr[3]) !== -1) {
  ...
  jumpToWeb(path, param) {
    ...
    var url = "https://" + config.main_host + path + "?shop_id=" + config.shop_id + "&user_id=" + cache.getCache('user_id') + param;
    this.navigateTo({
      url: '/pages/webview/webview?url=' + encodeURIComponent(url),
    })
  }
