...
import com.vpu.jmd.table.main.tables.records.MpOperateLogRecord;
...
import com.vpu.jmd.service.wechat.bean.ma.WxContentTemplate;
...
import static com.vpu.jmd.table.main.Tables.MP_OPERATE_LOG;
import static com.vpu.jmd.table.main.Tables.MP_VERSION;
...
public class MpOperateLogService extends MainBaseService {
	 ...
	 * @return
	 */
	public Map<String, Integer> getOperateFailLogQuery(Timestamp failStartTime, Timestamp failEndTime) {
		SelectConditionStep<Record2<String, Integer>> select = db()
				.select(MP_OPERATE_LOG.APP_ID, DSL.count(MP_OPERATE_LOG).as("failcount")).from(MP_OPERATE_LOG)
				.where(MP_OPERATE_LOG.OPERATE_TYPE.eq(OP_TYPE_AUDIT_FAILED));
		if (failStartTime != null) {
			select.and(MP_OPERATE_LOG.CREATE_TIME.ge(failStartTime));
		}
		if (failEndTime != null) {
			select.and(MP_OPERATE_LOG.CREATE_TIME.le(failStartTime));
		}
		select.groupBy(MP_OPERATE_LOG.APP_ID);
		return select.fetchMap(MP_OPERATE_LOG.APP_ID, DSL.count(MP_OPERATE_LOG).as("failcount"));
	}

	public Table<Record2<String, Integer>> getFailCoutSQL(Timestamp failStartTime, Timestamp failEndTime) {
		SelectConditionStep<Record2<String, Integer>> select = db()
				.select(MP_OPERATE_LOG.APP_ID.as("olAppId"), DSL.count(MP_OPERATE_LOG).as("failCount")).from(MP_OPERATE_LOG)
				.where(MP_OPERATE_LOG.OPERATE_TYPE.eq(OP_TYPE_AUDIT_FAILED));
		if (failStartTime != null) {
			select.and(MP_OPERATE_LOG.CREATE_TIME.ge(failStartTime));
		}
		if (failEndTime != null) {
			select.and(MP_OPERATE_LOG.CREATE_TIME.le(failEndTime));
		}
		select.groupBy(MP_OPERATE_LOG.APP_ID);
		Table<Record2<String, Integer>> table = select.asTable("ol");
		return table;
	}
}
