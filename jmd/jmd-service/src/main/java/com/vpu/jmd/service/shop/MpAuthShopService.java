...
import com.vpu.jmd.table.main.tables.records.BackProcessRecord;
import com.vpu.jmd.table.main.tables.records.MpAuthShopRecord;
import com.vpu.jmd.table.main.tables.records.MpDeployHistoryRecord;
import com.vpu.jmd.table.main.tables.records.MpOfficialAccountUserRecord;
import com.vpu.jmd.table.main.tables.records.MpVersionRecord;
import com.vpu.jmd.table.main.tables.records.ShopRecord;
import com.vpu.jmd.table.main.tables.records.UserRecord;
...
public class MpAuthShopService extends MainBaseService {
	...
	@Autowired
	private MpAuditService mpAuditService;
	...
	public WxOpenResult setSubMerchant(MpDeployQueryParam param) {
		...
		String success = String.valueOf(JsonResultCode.CODE_SUCCESS.getCode());
		...
		case 0:
			...
			if(execute3>0) {
				wxOpenResult.setErrcode(success);
			}else {
		...
		if(wxOpenResult.getErrcode().equals(success)) {
			...
			if(mpRecord!=null) {
				...
				if(inMessage.getEvent().equals("weapp_audit_success")) {
					...
					try {
						...
						mpAuditService.recordWechatAuditResult(appId, bindTemplateId, true, "");
						//return true
					} catch (WxErrorException e) {
						e.printStackTrace();
					}
					...
					mpAuditService.recordWechatAuditResult(appId, bindTemplateId, false, inMessage.getReason());
				}
			}
		}
	...
	public JsonResultCode liveCanCommitCode(MpAuthShopRecord record) {
		boolean checkHasLive = checkHasLive(record);
		if(!checkHasLive) {
			//缺少小程序直播权限集，请点击重新授权
			return JsonResultCode.WX_NO_LIVE_POWER;
		}

		Byte plugin = getPlugin(record.getAppId());
		if(plugin.equals((byte)1)||plugin.equals((byte)2)) {
			//当前无直播插件，请到微信公众平台开通直播
			return JsonResultCode.WX_NO_LIVE_PLUS;
		}
		return JsonResultCode.CODE_SUCCESS;
	}
	 ...
	 * @return
	 */
	public JsonResultCode uploadApplyAudit(MpUploadAuditParam param,Integer shopId) {
		MpAuthShopRecord mp = getAuthShopByShopId(shopId);
		Byte source = param.getSource();
		if(null==mp) {
			//店铺没有绑定小程序
			return JsonResultCode.WX_MA_SHOP_HAS_NO_AP;
		}
		Boolean isLiveCommit = param.getIsLiveCommit();
		if (null != isLiveCommit && isLiveCommit) {
			source=(byte)2;
			JsonResultCode code = liveCanCommitCode(mp);
			if(!code.equals(JsonResultCode.CODE_SUCCESS)) {
				return code;
			}
		}
		SystemAuditMsgVo audit = mpAuditService.createAudit(mp.getAppId(), source, null);
		if(audit.getErrcode()!=0) {
			return audit.getErrmsg();
		}
		return JsonResultCode.CODE_SUCCESS;
	}
}
