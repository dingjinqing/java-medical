...
import com.vpu.mp.service.pojo.shop.config.trade.h5.OrderProcessConfigParam;
...
import static com.vpu.mp.service.pojo.shop.config.trade.TradeConstant.EMPTY_LIST;
...
public class TradeService extends BaseShopConfigService {
     ...
     * 提醒发货 0不提醒 1提醒
     */
    final public static String K_ORDER_REMIND = "orderRemind";
    /**
     * 自定义信息标题：限制输入不超过6个字
     */
     ...
     * Sets orderRemind.
     *
     * @param orderRemind the orderRemind
     * @return the orderRemind
     */
    public int setOrderRemind(Byte orderRemind) {
        Assert.isTrue(orderRemind == (byte) 0 || orderRemind == (byte) 1,"Value must meet requirements");
        return this.set(K_ORDER_REMIND, orderRemind, Byte.class);
    }

    /**
     * Gets orderRemind title.
     *
     * @return the orderRemind title
     */
    public Byte getOrderRemind() {
        return this.get(K_ORDER_REMIND, Byte.class, Byte.valueOf("0"));
    }
     ...
     *              {@value com.vpu.mp.service.pojo.shop.config.trade.TradeConstant#FIELD_CLAZZ}
     */
    public void updateOrderProcess(OrderProcessConfigParam param) {
        try {
            BeanInfo beanInfo = Introspector.getBeanInfo(param.getClass());
            PropertyDescriptor[] descriptors = beanInfo.getPropertyDescriptors();
            Arrays.stream(descriptors)
     ...
     * 获取交易流程配置
     */
    public OrderProcessConfigParam getOrderFlowConfig(){
        OrderProcessConfigParam config =new OrderProcessConfigParam();
        try {
            BeanInfo beanInfo = Introspector.getBeanInfo(OrderProcessConfigParam.class);
            PropertyDescriptor[] propertyDescriptors = beanInfo.getPropertyDescriptors();
            Arrays.stream(propertyDescriptors)
                .filter(des -> !FIELD_CLAZZ.equalsIgnoreCase(des.getName()))
                .forEach((e) -> selectInvoke(e, config, this));
        } catch (IntrospectionException e) {
            e.printStackTrace();
        }
        return config;
