...
import com.vpu.jmd.table.main.tables.records.ShopVersionRecord;
import com.vpu.jmd.jedis.JedisKeyConstant;
import com.vpu.jmd.jedis.JedisManager;
import com.vpu.jmd.service.base.MainBaseService;
import com.vpu.jmd.service.base.bo.PageResult;
import com.vpu.jmd.service.shop.bo.ShopVersionPojo;
import com.vpu.jmd.service.shop.bo.ShopVersionVo;
import com.vpu.jmd.service.shop.bo.VersionEditParam;
import com.vpu.jmd.service.shop.bo.VersionListQueryParam;
...
import static com.vpu.jmd.table.main.Tables.SHOP_VERSION;
...
public class ShopVersionService extends MainBaseService {
	...
	public Integer editVersion(VersionEditParam vParam) {
        ...
        if (db().executeUpdate(record) > 0) {
            //清除装修页面的缓存
            String key = String.format(JedisKeyConstant.PAGE_DECORATE, vParam.getShopId(), "*");
            Set<String> pageKeys = jedisManager.keys(key);
            jedisManager.delete(pageKeys);
        }
        return 1;
    }
		...
		for (Field field : fields) {
			...
			try {
				...
				for (String string : list) {
					...
					for (String enNameInner : split) {
						if (enNameInner.equals(string)) {
							return true;
						}
					}
				}
