<template>
  ...
  >
    <p>
      {{ $t('order.orderExportConfirmTip_1') }}{{ totalRows
      }}{{ $t('order.orderExportConfirmTip_2') }}
    </p>
    ...
    >
      <div v-if="ok(key, item)">
        <div v-if="key === 'orderStatus'">
          {{ $t('orderSearch.' + key) }}:
          <span v-for="status in item" :key="status">
            {{ orderStatusMap.get(status) }}
          </span>
        </div>
        <div v-else-if="key === 'goodsType'">
          {{ $t('orderSearch.' + key) }}:
          <span v-for="type in item" :key="type">
    ...
    <div v-if="totalRows > 0" style="margin-top: 10px">
      <div style="margin-bottom: 10px">
        {{ $t('order.orderExportLimitTip') }}
      </div>
      <el-input-number
    ...
    <span slot="footer" class="dialog-footer">
      <el-button @click="showNodes = false" size="small">{{
...
import { orderExport, getExportTotalRows } from '@/api/store/order.js'
