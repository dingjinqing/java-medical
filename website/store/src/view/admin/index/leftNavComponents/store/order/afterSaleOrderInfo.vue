<template>
  <div class="content">
    <div class="main">
      <div class="item_box">
        <h2 class="h2_title">
          {{ returnTypeMap.get(returnInfo.returnType) + $t('order.applyText') }}
        </h2>
        <div class="refund_status_desc">
          <template v-if="returnInfo.returnType == 1">
            <template v-if="returnInfo.refundStatus == 1">
              ...
              <div class="btn_box">
                <el-button type="primary" size="small" @click="showRefund">{{
                  $t('order.refundBtn_1_1_1')
                }}</el-button>
                <el-button type="default" size="small" @click="showRefusal">{{
                  $t('order.refundBtn_1_1_2')
                }}</el-button>
              </div>
            </template>
            <template v-else-if="returnInfo.refundStatus == 2">
              ...
              <div class="btn_box">
                <el-button type="primary" size="small" @click="showRefund">{{
                  $t('order.refundBtn_1_2_1')
                }}</el-button>
                <el-button type="default" size="small" @click="showRefusal">{{
                  $t('order.refundBtn_1_2_2')
                }}</el-button>
              </div>
            </template>
            ...
            <template v-else-if="returnInfo.refundStatus == 4">
              ...
              <div class="btn_box">
                <el-button type="primary" size="small" @click="showRefund">{{
                  returnInfo.showRefundFailInfo === 1
                    ? '继续退款'
                    : $t('order.refundBtn_1_4_1')
                }}</el-button>
                <el-button
                  type="default"
                  size="small"
            ...
            <template
              v-else-if="
                returnInfo.refundStatus == 7 && returnInfo.operatorLastType == 1
              "
            >
              <div class="refund_description">
                <h3>
            ...
            <template
              v-else-if="
                returnInfo.refundStatus == 7 && returnInfo.operatorLastType == 2
              "
            >
              <div class="refund_description">
                <h3>
            ...
            <template v-if="returnInfo.refundStatus == 4">
              ...
              <div class="btn_box">
                <el-button type="primary" size="small" @click="showRefund">{{
                  returnInfo.showRefundFailInfo === 1
                    ? '继续退款'
                    : $t('order.refundBtn_4_1', [
                        returnTypeMap.get(returnInfo.returnType),
                      ])
                }}</el-button>
                <el-button
                  type="default"
      ...
      <div class="item_box" v-if="returnInfo.showRefundFailInfo === 1">
        ...
        <div class="return-fail-info">
          <div v-if="returnInfo.successMoney">
            退款成功：￥{{ returnInfo.successMoney }}
          </div>
          <div v-if="returnInfo.failMoney">
            退款失败：￥{{ returnInfo.failMoney }}
          </div>
          <div v-if="returnInfo.failDesc">
            退款失败原因：{{ returnInfo.failDesc }}
          </div>
        </div>
      </div>
        ...
        <table class="refund_info_table">
          <tbody>
                ...
                <el-popover placement="right" trigger="hover">
                  ...
                  >
                    ...
                    <div class="brief-info">
                      ...
                      <div>
                        配送方式：{{
                          orderBriefData.deliverType === 0 ? '快递' : '自提'
                        }}
                      </div>
            ...
            <tr>
              ...
              <td colspan="3">
                <template v-for="goods in returnInfo.returnGoods">
                  <div class="goods_info" :key="goods.recId">
                    <img :src="$imageHost + '/' + goods.goodsImg" alt="" />
            ...
            <tr>
              ...
              <td colspan="3">
                <span
                  >{{ returnInfo.username }} {{ returnInfo.mobile }}</span
                >
              </td>
            </tr>
            <tr>
              ...
              <td colspan="3">
                {{ returnTypeMap.get(returnInfo.returnType) }} ({{
                  $t(`order.returnWaysList`).find(
                    (item) => item.value === returnInfo.returnSource
                  ).label
                }})
              </td>
            </tr>
                ...
                {{
                  ...
                  null
                    ? returnStatusToShowMapping[
                        returnInfo.refundStatus + '-' + returnInfo.returnType
                      ]
                    : returnStatusToShowMapping[returnInfo.refundStatus + '']
                }}
              </td>
            ...
            <tr>
              ...
              <td colspan="3" class="refund_info_td">
                <template
                  v-if="
                    returnInfo.refundStatus == 2 || returnInfo.refundStatus == 4
                  "
                >
                  <p class="refund_info">
                    ...
                    ></el-input-number>
                    {{
                      getCurrencyPool_0 + '，' + $t('order.returnShippingFee')
                    }}：
                    <el-input-number
                      v-model="shippingFee"
                      size="small"
                    ...
                    {{
                      getCurrencyPool_0 +
                      '，' +
                      $t('order.maxRefundShippingFee') +
                      (returnInfo.canReturnShippingFee == null
                        ? 0.0
                        : returnInfo.canReturnShippingFee.toFixed(2) +
                          getCurrencyPool_0)
                    }}
                  </p>
                  <p>
                    ...
                    <span class="text-warning refund-money">{{
                    ...
                    <template v-if="returnInfo.orderInfo.bkOrderMoney > 0">
                      <span
                        >{{
                          $t('order.bk_order_money') + '：' + getCurrencyPool_1
                        }}
                        <span class="text-warning refund-bk-order-money">{{
                          bk_order_money.toFixed(2)
                        }}</span>
                        +
                      </span>
                    </template>
                ...
                <template v-else>
                  <p class="refund_info">
                    {{ $t('order.refundGoodsPrice') }}：<span
                      class="text-warning"
                    >
                      {{ getCurrencyPool_1 + returnInfo.money }}</span
                    >
                  </p>
                  <p class="refund_info">
                    {{ $t('order.shippingFee') }}：<span class="text-warning">{{
                      getCurrencyPool_1 + returnInfo.shippingFee
                    }}</span>
                  </p>

                  <span v-if="returnInfo.refundStatus == 5" class="refund_info">
                    {{ $t('order.returnInfo') }}：
                    <span
            ...
            <template
              v-if="returnInfo.refundStatus == 4 && returnInfo.returnType == 1"
            >
              <tr>
              ...
              <tr>
                ...
                <td colspan="3">
                  {{ returnInfo.shippingNo }}
                  <a class="text-info" target="blank" :href="toShippingView">{{
                    $t('order.select')
                  }}</a>
                </td>
              </tr>
            ...
            <tr>
              <td>
                <span class="w2">{{ $t('order.description') }}</span>
              </td>
              <td colspan="3">
      ...
      <div class="item_box" v-if="1 != 1">
        <h2 class="h2_title">{{ $t('order.helpBuyInfo') }}</h2>
        <template>
          <el-table :data="returnInfo.helpBuyInfo" style="width: 100%">
            <el-table-column
            ...
            <el-table-column prop="finishTime" :label="$t('order.finishTime')">
            </el-table-column>
          </el-table>
        </template>
      </div>
            ...
            <div class="bottom">
              <ul>
                <template v-if="record.logicStatus == 1">
                  ...
                  <li>
                    {{ $t('order.returnReasonText') }}：{{
                      $t('order.reasonTypeList')[returnInfo.reasonType]
                    }}
                  </li>
                  <li>
                    {{ $t('order.returnMoney') }}：{{
                      (returnInfo.money + returnInfo.shippingFee).toFixed(2)
                    }}
                  </li>
                  <li>
                ...
                <template v-else-if="record.logicStatus == 2">
                  ...
                  <li>
                    {{ $t('order.refusedReason') }}：{{
                      returnInfo.refundRefuseReason
                    }}
                    {{ returnInfo.applyNotPassReason }}
                  </li>
                </template>
                ...
                <template v-else-if="record.logicStatus == 4">
                  ...
                  <li>
                    {{
                      $t('order.passReturnApplyNote1', {
                        returnAddress: returnInfo.returnAddress,
                        consignee: returnInfo.consignee,
                        merchantTelephone: returnInfo.merchantTelephone,
                      })
                    }}
                  </li>
                </template>
                ...
                <template v-else-if="record.logicStatus == 8">
                  ...
                  <li>
                    {{ $t('order.logisticsVoucherImages') + '：' }}
                    <a
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="refusalConfirm">确 定</el-button>
      </span>
    </el-dialog>
    <el-dialog
      title="确认退货完成"
    ...
    >
      <div>
        <template
          v-if="returnInfo.returnType == 1 && returnInfo.refundStatus == 1"
        >
          <el-form
          ...
          >
            <el-form-item :label="$t('order.consignee')" prop="consignee">
              <el-input
            ...
            <el-form-item :label="$t('order.phone')" prop="merchantTelephone">
              <el-input
        ...
        <template
          v-else-if="
            returnInfo.refundStatus == 4 || returnInfo.refundStatus == 2
          "
        >
          <span style="display: inline-block; width: 100%">
            ...
            <span
              ...
              <span style="color: #f66"
                >确认已收到退货实物或者协商通过后再操作退货完成，本操作会同步退款给顾客(货款退回到顾客支付的账户)，请谨慎操作</span
              ></span
            >
          </span>
          <span>
            由于支付渠道（如跨行退款）的延迟，会在
            <span style="color: #f66">3个工作日内到账，</span>
            <P>您确定要对订单操作退货完成吗？</P>
          </span>
        </template>
        <template v-else>
...
import {
  returnInfo, handleReturnInfo, getOrderBrief
