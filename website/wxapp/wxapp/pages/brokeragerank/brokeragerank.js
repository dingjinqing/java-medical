// pages/brokeragerank/brokeragerank.js
...
global.wxPage({
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarColor({
      frontColor: '#ffffff',
      backgroundColor: this.data.comColor,
    });
    var that = this;
    wx.hideShareMenu();
    rank_request(that)
  ...
  onShareAppMessage: function () {
      return{
        path: "/pages/brokeragerank/brokeragerank?user_id=" + util.getCache('user_id') + "&invite_id=" + util.getCache('user_id')
      }
  }
})
  ...
  util.api('/api/wxapp/rebate/ranking', function (res) {
    ...
    if (rank_info.rebate_list && rank_info.rebate_list!="") {
      for (var i in rank_info.rebate_list) {
        if (parseFloat(rank_info.rebate_list[i].total_money) > 100000) {
          rank_info.rebate_list[i].total_money = parseFloat(rank_info.rebate_list[i].total_money).toFixed(0);
        }
      }
    }
    ...
    if (rank_info.rebate_top_three && rank_info.rebate_top_three!="") {
      for (var i = 0; i < rank_info.rebate_top_three.length; i++) {
        if (parseFloat(rank_info.rebate_top_three[i].total_money) > 100000) {
          rank_info.rebate_top_three[i].total_money = parseFloat(rank_info.rebate_top_three[i].total_money).toFixed(0);
        }
      }
    }
    that.setData({
