...
public class UserPatientCoupleDao  extends ShopBaseDao {
    ...
    public List<PatientOneParam> listPatientIdsByUser(Integer userId) {
        List<PatientOneParam> patientList = db().select(USER_PATIENT_COUPLE.asterisk(),PATIENT.MOBILE,PATIENT.IDENTITY_CODE,PATIENT.IDENTITY_TYPE).from(USER_PATIENT_COUPLE)
