...
CREATE TABLE `b2c_ali_mini_agent` (
  ...
  `create_agent_time` timestamp NULL DEFAULT NULL COMMENT '事务初始时间',
  `app_name` varchar(191) DEFAULT NULL COMMENT '代商家创建的小程序应用名称',
  `app_english_name` varchar(191) DEFAULT NULL COMMENT '小程序英文名称，长度3~20个字符	',
  `app_category_ids` varchar(191) DEFAULT NULL COMMENT '小程序应用类目，参数格式：一级类目_二级类目。',
  `app_slogan` varchar(191) DEFAULT NULL COMMENT '代商家创建的小程序的简介，请用一句话简要描述小程序提供的服务；应用上架后一个自然月可修改5次（10~32个字符）	',
  ...
  `app_logo` varchar(191) DEFAULT NULL COMMENT '商家小程序应用图标，最大256KB,png、jpeg、jpg格式，建议上传像素为180*180',
  `app_desc` varchar(191) DEFAULT NULL COMMENT '商家小程序描述信息，简要描述小程序主要功能（20-200个字）',
  `create_mini_result` text COMMENT '创建小程序接口结果,json',
  `create_mini_time` timestamp NULL DEFAULT NULL COMMENT '创建小程序时间',
  `user_id` varchar(191) DEFAULT NULL COMMENT '授权商户的user_id	',
  `auth_app_id` varchar(191) DEFAULT NULL COMMENT '授权商户的appid	',
  `confirm_agent_result` text COMMENT '提交事务接口结果,json',
  `confirm_agent_time` timestamp NULL DEFAULT NULL COMMENT '提交事务时间',
  PRIMARY KEY (`rec_id`),
  UNIQUE KEY `shop_id` (`shop_id`),
  UNIQUE KEY `auth_app_id` (`auth_app_id`)
)COMMENT='阿里小程序授权信息';
...
CREATE TABLE `b2c_ali_mini_app_version` (
  `rec_id` int(11) NOT NULL,
  `app_version` varchar(191) DEFAULT NULL COMMENT '小程序版本号',
  `app_version_desc` varchar(191) DEFAULT NULL COMMENT '小程序版本描述',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`rec_id`),
  UNIQUE KEY `app_version` (`app_version`)
);



CREATE TABLE `b2c_ali_mini_auth_shop` (
  `auth_app_id` varchar(191) NOT NULL COMMENT '授权商户的appid	',
  `user_id` varchar(191) DEFAULT NULL COMMENT '授权商户的user_id	',
  `app_auth_token` varchar(191) DEFAULT NULL COMMENT '应用授权令牌	',
  `app_refresh_token` varchar(191) DEFAULT NULL COMMENT '刷新令牌',
  ...
  `app_version` varchar(191) DEFAULT NULL COMMENT '小程序版本',
  `status` varchar(191) DEFAULT NULL COMMENT '小程序版本状态，INIT-开发中，AUDITING-审核中，WAIT_RELEASE-审核通过，AUDIT_REJECT-审核驳回，RELEASE-已上架，GRAY-灰度中，OFFLINE-下架	',
  ...
  `exp_qr_code_url` varchar(191) DEFAULT NULL COMMENT '小程序体验版二维码地址	',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `b2c_ali_my_order` (
  ...
  `order_item_num` int(11) DEFAULT '0' COMMENT '门店数量	',
  `total_price` decimal(10,2) DEFAULT '0.00' COMMENT '总价格	',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `order_ticket` varchar(191) DEFAULT NULL COMMENT '订单授权码，用于事务创建接口中的入参',
  `service_code` varchar(191) DEFAULT NULL COMMENT '服务码，用于商户和ISV匹配订单',
  `link_shop_id` int(11) DEFAULT NULL COMMENT '关联店铺Id',
  PRIMARY KEY (`rec_id`),
  UNIQUE KEY `commodity_order_id` (`commodity_order_id`),
  UNIQUE KEY `link_shop_id` (`link_shop_id`)
)COMMENT='阿里小程序模板订购信息';
...
CREATE TABLE `b2c_ali_user_token` (
  ...
  `expires_in_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '支付宝用户访问令牌过期时间',
  `refresh_token` varchar(191) NOT NULL DEFAULT '' COMMENT '支付宝用户刷新令牌',
  `re_expires_in_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '支付宝用户刷新令牌过期时间',
  PRIMARY KEY (`rec_id`),
  UNIQUE KEY `app_id` (`app_id`,`user_id`)
)COMMENT='小程序版本列表';
...
)COMMENT='定时任务执行结果记录表（只记录执行失败的记录）';
...
CREATE TABLE `b2c_goods` (
  ...
  `goods_desc` text COLLATE utf8mb4_unicode_ci,
...
CREATE TABLE `b2c_goods_brand` (
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '记录修改时间',
...
CREATE TABLE `b2c_mp_deploy_history` (
  `deploy_id` int(11) NOT NULL COMMENT '自增ID',
...
CREATE TABLE `b2c_mp_operate_log` (
  `operate_id` int(11) NOT NULL COMMENT '自增ID',
  `app_id` varchar(191) NOT NULL DEFAULT '' COMMENT '小程序app_id',
...
CREATE TABLE `b2c_shop` (
  ...
  `shop_license` varchar(191) DEFAULT NULL COMMENT '营业执照',
  `shop_tax_credential` varchar(191) DEFAULT NULL COMMENT '税务登记证',
...
CREATE TABLE `b2c_shop_account` (
  `sys_id` int(11) NOT NULL COMMENT '店铺ID',
...
CREATE TABLE `b2c_shop_question_feedback` (
  `question_feedback_id` int(11) NOT NULL,
