...
`first_special_id` INT ( 11 ) DEFAULT '0' COMMENT '首单特惠ID',
`free_ship` TINYINT ( 1 ) DEFAULT '0' COMMENT '满包邮',
...
create table `b2c_order_goods_rebate` (
  ...
  `rec_id`             mediumint(8)             not null default '0' comment '商品行ID',
  ...
  `real_rebate_money`  decimal(10, 4)       not null default 0.0000 comment '实际返利金额',
  `create_time`		timestamp      	default current_timestamp,
  `update_time` 	timestamp      	default current_timestamp on update current_timestamp comment '最后修改时间',
