...
(
    ...
    `update_time`          timestamp               default current_timestamp on update current_timestamp comment '最后修改时间',
     ...
     `random_min`           decimal(10, 2)          not null      default 0 comment '分裂优惠卷随机金额最低',
      `random_max`          decimal(10, 2)          not null      default 0 comment '分裂优惠卷随机金额最高',
      `receive_per_num`     tinyint(1)      not null      default 0 comment '分裂优惠券领券人数是否限制 0不限制 1限制',
...
(
    ...
    `discount_brand_id` varchar(299)  DEFAULT NULL COMMENT '折扣品牌ID',
...
(
    ...
    `services_number`      int(11)                 default 0 comment '服务数量',
    `tech_services_number` int(11)  default  0  comment '技师单时段服务数量',
...
(
    ...
    `status`             tinyint(2)   not null default '1' comment '状态：1启用 0停用',
...
(
    ...
    `first_rebate` TINYINT(1) DEFAULT 0  NOT NULL   COMMENT '邀请新用户下首单返利',
    `first_ratio`  FLOAT DEFAULT 0  NULL   COMMENT '首单返利金额',
    `first_ratio_2`  FLOAT DEFAULT 0  NULL   COMMENT '首单返利金额',
    `first_ratio_3`  FLOAT DEFAULT 0  NULL   COMMENT '首单返利金额',
    `first_ratio_4`  FLOAT DEFAULT 0  NULL   COMMENT '首单返利金额',
    `first_ratio_5`  FLOAT DEFAULT 0  NULL   COMMENT '首单返利金额',
...
`goods_sales` decimal(10, 2)  NULL DEFAULT 0.0 comment '销售额',
`recommend_user_num` int(11)  NULL DEFAULT 0 comment '推荐人数',
`collect_use_num` int(11)  NULL DEFAULT 0 comment '收藏人数',
`share_pv` int(11)  NULL DEFAULT 0 comment '分享次数',
`share_uv` int(11)  NULL DEFAULT 0 comment '分享人数',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后修改时间',
  PRIMARY KEY (`id`) ,
...
(
    ...
    `old_paid_money`       decimal(10,2)  DEFAULT NULL COMMENT '成交新老客户支付金额',
...
(
    ...
    `act_rule`        text           null comment '活动规则',
...
(
    ...
    `recommend_sort_id`  text COMMENT '指定商家分类可用',
    `status`             TINYINT(1) DEFAULT '1' COMMENT '0停用',
    `create_time`        TIMESTAMP  DEFAULT CURRENT_TIMESTAMP,
    `update_time`        TIMESTAMP  DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最后修改时间',
    `del_flag`           TINYINT(1) DEFAULT '0' COMMENT '1删除',
...
(
    ...
    `goods_ids`         varchar(200) NULL     DEFAULT NULL COMMENT '触发条件为2时：分享指定商品id列表，逗号分隔符',
