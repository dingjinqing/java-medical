...
public class RealTimeOverviewService extends ShopBaseService {
    ...
    public List<Tuple2<Byte,Double>> cumulativeSum(List<Tuple2> trades){
        double sum = 0;
        byte hour = 0;
        List<Tuple2<Byte,Double>> result = new ArrayList<>(24);
        for(Tuple2 tuple : trades){
    ...
    public SelectConditionStep<Record8<java.sql.Date, Integer, Integer, Integer, Integer, Integer, Double, Double>> getConditonSelect(CoreIndicatorParam param){
        UserSummaryTrend us = UserSummaryTrend.USER_SUMMARY_TREND.as("us");
