...
public class OrderGoodsService extends ShopBaseService{
    ...
    public Map<String, List<ApiOrderGoodsListVo>> getGoodsByOrderSns(List<String> orderSns) {
        return db().select(TABLE.ORDER_SN, TABLE.REC_ID,TABLE.PRODUCT_ID,TABLE.PRODUCT_SN,TABLE.GOODS_ID,TABLE.GOODS_SN,TABLE.GOODS_NAME,TABLE.GOODS_ATTR,TABLE.GOODS_PRICE, TABLE.DISCOUNTED_GOODS_PRICE,TABLE.GOODS_NUMBER,TABLE.REFUND_STATUS,TABLE.SEND_NUMBER)
            .from(TABLE)
            .where(TABLE.ORDER_SN.in(orderSns))
