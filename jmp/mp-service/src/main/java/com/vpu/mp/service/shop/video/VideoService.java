package com.vpu.mp.service.shop.video;
...
public class VideoService extends ShopBaseService {
	...
	public UpYun getUpYunClient() {
		return new UpYun(upYunConfig.getVideoServer(), upYunConfig.getVideoOpName(), upYunConfig.getVideoOpPassword());
	}

	public UpyunSynVideo getUpyunSynVideo() {
		return new UpyunSynVideo(upYunConfig.getVideoServer(), upYunConfig.getVideoOpName(),
	 ...
	 * @throws Exception
	 */
	public UploadedVideoRecord uploadVideoFile(String name, Integer size, InputStream inStream, String ext,
			Integer catId,
			Integer userId)
			throws IOException, Exception {
		String videoPath = String.format("/video/%s/%s%s", DateUtil.dateFormat(DateUtil.DATE_FORMAT_SHORT),
				Util.randomId(), ext);
		...
		String snapshot = String.format("/snapshot/%s/%s.jpg", DateUtil.dateFormat(DateUtil.DATE_FORMAT_SHORT),
				Util.randomId());
		synVideo.snapshot(videoPath, snapshot, "00:00:00");

		return this.insertVideoRecord(name, videoPath, size,
				catId, userId, avMeta, videoStream, snapshot, ext);
	}
	 ...
	 * @return
	 */
	protected UploadedVideoRecord insertVideoRecord(String name, String videoPath, Integer size,
			Integer catId, Integer userId, AvMeta avMeta, Stream videoStream, String snapshot, String ext) {
	...
	public PageResult<UploadVideoCatNameVo> getPageList(VideoListQueryParam param) {
		SelectWhereStep<Record> select = db().select(UPLOADED_VIDEO.asterisk(), UPLOADED_VIDEO_CATEGORY.VIDEO_CAT_NAME)
				.from(UPLOADED_VIDEO)
		...
		PageResult<UploadVideoCatNameVo> result = this.getPageResult(select, param.page, param.pageRows,
	 ...
	 * @return
	 */
	public SelectWhereStep<Record> buildOptions(SelectWhereStep<Record> select, VideoListQueryParam param) {
		if (param == null) {
			return select;
		}
		Byte noDel = 0;
		select.where(UPLOADED_VIDEO.DEL_FLAG.eq(noDel))
		...
		SortField<?>[] sortFields = { UPLOADED_VIDEO.CREATE_TIME.desc(), UPLOADED_VIDEO.CREATE_TIME.asc(),
				UPLOADED_VIDEO.VIDEO_SIZE.desc(), UPLOADED_VIDEO.VIDEO_SIZE.asc(), UPLOADED_VIDEO.VIDEO_NAME.desc(),
	...
	public Integer getAllSize() {
		Record1<BigDecimal> rec = db().select(DSL.sum(UPLOADED_VIDEO.VIDEO_SIZE).as("total"))
				.from(UPLOADED_VIDEO)
	...
	public void deleteUpYunOvertimeVideo() {
		Result<UploadedVideoRecord> records = db().fetch(UPLOADED_VIDEO, UPLOADED_VIDEO.DEL_FLAG.eq((byte) 1)
				.and(UPLOADED_VIDEO.VIDEO_CAT_ID.eq(-1))
				.and(UPLOADED_VIDEO.UPYUN_DEL.eq((byte) 0))
				.and(UPLOADED_VIDEO.UPDATE_TIME.lt(DateUtil.getDalyedDateTime(-24 * 3600 * 30))));
		for (UploadedVideoRecord record : records) {
			try {
				this.deleteYpYunVideo(record.getVideoPath());
				db().update(UPLOADED_VIDEO).set(UPLOADED_VIDEO.UPYUN_DEL, (byte) 1)
	 ...
	 */
	public Object[] validVideoParam(UploadVideoParam param, Part file) throws IOException {
		Integer maxSize = 10 * 1024 * 1024;
		if (file.getSize() > maxSize) {
			return new JsonResultCode[] { JsonResultCode.CODE_VIDEO_UPLOAD_GT_10M };
		}
		if (!validVideoContentType(file.getContentType())) {
			return new JsonResultCode[] { JsonResultCode.CODE_VIDEO_FORMAT_INVALID };
		}
		return null;
	}
