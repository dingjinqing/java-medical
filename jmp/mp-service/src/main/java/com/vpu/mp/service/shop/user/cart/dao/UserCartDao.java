...
public class UserCartDao extends ShopBaseService {
	...
	public List<Integer> getUserIdFromCartStartTime(Timestamp time) {
		return db().selectFrom(USER_CART_RECORD).where(USER_CART_RECORD.CREATE_TIME.ge(time))
				.groupBy(USER_CART_RECORD.USER_ID).fetch().getValues(USER_CART_RECORD.USER_ID, Integer.class);
	}
	...
	public List<Integer> getUserIdUtilToCartEndTime(Timestamp time) {
		return db().selectFrom(USER_CART_RECORD).where(USER_CART_RECORD.CREATE_TIME.le(time))
				.groupBy(USER_CART_RECORD.USER_ID).fetch().getValues(USER_CART_RECORD.USER_ID, Integer.class);
	}
}
