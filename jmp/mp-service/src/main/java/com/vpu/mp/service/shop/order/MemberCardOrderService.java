...
public class MemberCardOrderService extends ShopBaseService {
    ...
    public PageResult<MemberCardVo> getMemberCardOrderList(MemberCardParam param) {
        SelectOnConditionStep<Record16<Integer, String, Integer, Byte, Timestamp, BigDecimal, BigDecimal, BigDecimal,
            ...
            db().select(VIRTUAL_ORDER.ORDER_ID, VIRTUAL_ORDER.ORDER_SN,
                ...
                VIRTUAL_ORDER.USE_ACCOUNT, VIRTUAL_ORDER.USE_SCORE, VIRTUAL_ORDER.RETURN_TIME, USER.USERNAME, USER.MOBILE,
    ...
    private void buildOptions(SelectOnConditionStep<Record16<Integer, String, Integer, Byte, Timestamp, BigDecimal,
        ...
        if (null != isRefund) {
            if (isRefund) {
                select.and(VIRTUAL_ORDER.RETURN_FLAG.eq(SUCCESS).or(VIRTUAL_ORDER.RETURN_FLAG.eq(MemberCardParam.FAILED)));
            } else {
                select.and(VIRTUAL_ORDER.RETURN_FLAG.eq(MemberCardParam.NOT_REFUND));
            }
        }
    ...
    public void memberCardOrderRefund(MemberCardRefundParam param) {
        ...
        String payCode = payInfo.get(VIRTUAL_ORDER.PAY_CODE);
        BigDecimal useAccount = payInfo.get(VIRTUAL_ORDER.USE_ACCOUNT);
        BigDecimal useMoney = payInfo.get(VIRTUAL_ORDER.MONEY_PAID);
        BigDecimal useScore = payInfo.get(VIRTUAL_ORDER.USE_SCORE);
        String orderSn = payInfo.get(VIRTUAL_ORDER.ORDER_SN);
        Integer userId = payInfo.get(VIRTUAL_ORDER.USER_ID);
        BigDecimal returnAccount = payInfo.get(VIRTUAL_ORDER.RETURN_ACCOUNT);
        BigDecimal returnMoney = payInfo.get(VIRTUAL_ORDER.RETURN_MONEY);
        BigDecimal returnScore = payInfo.get(VIRTUAL_ORDER.RETURN_SCORE);
        ...
        // 更新订单状态
        db().update(VIRTUAL_ORDER).set(VIRTUAL_ORDER.RETURN_FLAG, SUCCESS).set(VIRTUAL_ORDER.RETURN_MONEY,
            finalReturnMoney).set(VIRTUAL_ORDER.RETURN_ACCOUNT, finalReturnAccount)
            .set(VIRTUAL_ORDER.RETURN_SCORE, finalReturnScore).execute();
    }
    ...
    private Record10<String, BigDecimal, BigDecimal, BigDecimal, String, Integer, Byte, BigDecimal, BigDecimal,
        ...
        Record10<String, BigDecimal, BigDecimal, BigDecimal, String, Integer, Byte, BigDecimal, BigDecimal
            , BigDecimal> order =
            db().select(VIRTUAL_ORDER.PAY_CODE, VIRTUAL_ORDER.MONEY_PAID, VIRTUAL_ORDER.USE_SCORE,
                VIRTUAL_ORDER.USE_ACCOUNT, VIRTUAL_ORDER.ORDER_SN, VIRTUAL_ORDER.USER_ID, VIRTUAL_ORDER.RETURN_FLAG,
                VIRTUAL_ORDER.RETURN_MONEY, VIRTUAL_ORDER.RETURN_ACCOUNT, VIRTUAL_ORDER.RETURN_SCORE)
                .from(VIRTUAL_ORDER)
                .where(VIRTUAL_ORDER.ORDER_ID.eq(orderId)).fetchOne();
        if (null == order) {
