...
public class StoreAccountService extends MainBaseService {
	 ...
	 * @return
	 */
	public PageResult<StoreAccountVo> accountList(Integer currentPage, Integer pageRows, Integer sysId,
			Integer shopId) {
		...
		return this.getPageResult(where, currentPage, pageRows, StoreAccountVo.class);
	}
	...
	public int edit(StoreAccountEditParam param) {
		...
		int execute = db().update(STORE_ACCOUNT).set(map).where(STORE_ACCOUNT.ACCOUNT_ID.eq(param.getAccountId())).execute();
		return execute;
	}
	
	public StoreAccountRecord findInfo(String accountName,String mobile,Integer accountId) {
		SelectConditionStep<StoreAccountRecord> where = db().selectFrom(STORE_ACCOUNT).where(STORE_ACCOUNT.ACCOUNT_NAME.eq(accountName).and(STORE_ACCOUNT.MOBILE.eq(mobile)));
		if(accountId!=null) {
