...
import static com.vpu.mp.service.foundation.util.DateUtil.DATE_FORMAT_SIMPLE;
...
public class FootPrintService extends ShopBaseService {
	...
	public List<FootprintListVo> getFootprintPage(Integer userId, String keyWord, Integer currentPages, Integer pageRows){
		...
		List<FootprintListVo> footprintList =new ArrayList<>();
		...
		// 安装日期分组
		footprintListVos.forEach(footprint->{
			FootprintListVo footOptional = footprintList.stream().filter(f ->f.getDate().equals(footprint.getDate())).findFirst()
					.orElseGet(()->{
						FootprintListVo newfootprint = new FootprintListVo();
						newfootprint.setDate(footprint.getDate());
						footprintList.add(newfootprint);
						return newfootprint;
			});
			footOptional.getGoodsList().addAll(footprint.getGoodsList());
        });
