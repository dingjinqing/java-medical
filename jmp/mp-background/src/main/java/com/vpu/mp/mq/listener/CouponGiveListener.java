...
public class CouponGiveListener implements BaseRabbitHandler {
    ...
    public void handler(@Payload CouponGiveQueueParam param, Message message, Channel channel) {
		List<Integer> rowsList = saas.getShopApp(param.getShopId()).coupon.couponGiveService.handlerCouponGive(param);
        //更新taskJob进度和状态
        saas.taskJobMainService.updateProgress(Util.toJson(param),param.getTaskJobId(),rowsList.size(),param.getUserIds().size()*param.getCouponArray().length);
    }
